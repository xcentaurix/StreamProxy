...
try:
    ...
    def is_powerset_domain(*args, **kwargs):
        return False
    enhanced_log(f"⚠️ LiveTV extractor non disponibile: {e}", "WARNING", "AppCore")
...
try:
    ...
    try:
        ...
        def default_backend():
            return None
        enhanced_log("⚠️ Nessuna libreria AES disponibile - decrittazione disabilitata", "WARNING", "AppCore")
...
def convert_fmp4_to_ts(fmp4_content, stream_id=None):
    ...
    try:
        if len(fmp4_content) < 8:
            enhanced_log("⚠️ [FMP4_CONVERT] Segmento troppo piccolo, uso fallback", "WARNING", "proxy_ts")
        ...
        if len(fmp4_content) > 0 and fmp4_content[0] == 0x47:
            enhanced_log("ℹ️ [FMP4_CONVERT] Contenuto già in formato TS", "INFO", "proxy_ts")
            return fmp4_content
        ...
        if len(fmp4_content) >= 8:
            ...
            if header_type not in [b'ftyp', b'styp', b'moof', b'mdat']:
                enhanced_log(f"⚠️ [FMP4_CONVERT] Header non riconosciuto: {header_type}, provo comunque", "WARNING", "proxy_ts")
        
        enhanced_log(f"🔄 [FMP4_CONVERT] Processamento {len(fmp4_content)} bytes fMP4", "DEBUG", "proxy_ts")
        ...
        if stream_id and stream_id in STREAM_KEY_INFO:
            ...
            if stream_info.get('is_freeshot', False):
                enhanced_log(f"✅ [FMP4_DIRECT] Freeshot: invio diretto fMP4 (ottimale per Enigma2)", "INFO", "proxy_ts")
                enhanced_log(f"📊 [FMP4_DIRECT] Dimensione: {len(fmp4_content)} bytes", "DEBUG", "proxy_ts")
                return fmp4_content
        ...
        if stream_id and stream_id in STREAM_KEY_INFO:
            ...
            if init_segment:
                enhanced_log(f"📦 [FMP4_CONVERT] Usando init segment: {len(init_segment)} bytes", "DEBUG", "proxy_ts")
                init_data = init_segment
            else:
                enhanced_log(f"⚠️ [FMP4_CONVERT] Init segment mancante per stream {stream_id}", "WARNING", "proxy_ts")
        ...
        enhanced_log("📺 [FMP4_CONVERT] Aggiunto PAT/PMT ottimizzato per Enigma2", "DEBUG", "proxy_ts")
        ...
        if init_data:
            ...
            enhanced_log(f"📦 [FMP4_CONVERT] Combinato init ({len(init_data)}) + payload ({len(fmp4_content)}) = {len(payload_data)} bytes", "DEBUG", "proxy_ts")
        else:
            # ⚠️ Senza init segment, il video potrebbe non funzionare
            payload_data = fmp4_content
            enhanced_log(f"⚠️ [FMP4_CONVERT] ATTENZIONE: Nessun init segment - video potrebbe non funzionare!", "WARNING", "proxy_ts")
        ...
        chunk_size = TS_PAYLOAD_SIZE - 19  # Spazio per PES header completo nel primo pacchetto
        ...
        while pos < len(payload_data):
            if packet_num == 0:
                ...
                if len(packet_payload) < TS_PAYLOAD_SIZE - adaptation_field_length:
                    packet_payload += b'\xFF' * (TS_PAYLOAD_SIZE - adaptation_field_length - len(packet_payload))
        ...
        for audio_frame in range(3):  # 3 frame audio per segmento
            ...
            audio_pts = (base_timestamp + audio_frame * 1920) & 0x1FFFFFFFF  # 1920 = 90000/46.875 (AAC frame duration)
        ...
        enhanced_log(f"✅ [FMP4_CONVERT] Creati {len(ts_packets)} pacchetti TS ({len(ts_stream)} bytes)", "INFO", "proxy_ts")
        enhanced_log(f"📊 [FMP4_CONVERT] Struttura: PAT + PMT + {len(ts_packets)-5} video + PCR", "DEBUG", "proxy_ts")
        
        # Verifica sync byte
        if ts_stream and ts_stream[0] == 0x47:
            enhanced_log("✅ [FMP4_CONVERT] Stream TS valido (sync byte 0x47)", "DEBUG", "proxy_ts")
        else:
            enhanced_log("❌ [FMP4_CONVERT] ERRORE: Stream TS non valido!", "ERROR", "proxy_ts")
        
        enhanced_log(f"✅ [FMP4_CONVERT] Conversione completata: {len(ts_stream)} bytes TS", "INFO", "proxy_ts")
        
        return ts_stream
        
    except Exception as e:
        enhanced_log(f"❌ [FMP4_CONVERT] Errore conversione: {e}", "ERROR", "proxy_ts")
        # ✅ FALLBACK MIGLIORATO: Stream TS completo e valido per Enigma2
        try:
            # PAT completo
            pat = bytes([0x47, 0x40, 0x00, 0x10, 0x00]) + \
                  bytes([0x00, 0xB0, 0x0D, 0x00, 0x01, 0xC1, 0x00, 0x00, 0x00, 0x01, 0xF0, 0x00, 0x2A, 0xB1, 0x04, 0xB2]) + \
                  b'\xFF' * 167
            
            # PMT completo con video + audio
            pmt = bytes([0x47, 0x50, 0x00, 0x10, 0x02]) + \
                  bytes([0x00, 0xB0, 0x17, 0x00, 0x01, 0xC1, 0x00, 0x00, 0xE1, 0x00, 0xF0, 0x00]) + \
                  bytes([0x1B, 0xE1, 0x00, 0xF0, 0x00, 0x0F, 0xE1, 0x01, 0xF0, 0x00, 0x2F, 0x44, 0xB9, 0x9B]) + \
                  b'\xFF' * 163
            
            # Pacchetto video con PES header completo e PTS
            pes_header = bytes([0x00, 0x00, 0x01, 0xE0, 0x00, 0x00, 0x84, 0x80, 0x05, 0x21, 0x00, 0x01, 0x00, 0x01])
            ...
            audio_pes = bytes([0x00, 0x00, 0x01, 0xC0, 0x00, 0x00, 0x84, 0x80, 0x05, 0x21, 0x00, 0x01, 0x00, 0x01])
            ...
            enhanced_log(f"🔧 [FMP4_CONVERT] Fallback TS completo generato: {len(fallback_ts)} bytes", "INFO", "proxy_ts")
            return fallback_ts
        except Exception as fallback_error:
            # Fallback di emergenza
            emergency_ts = b'\x47\x1F\xFF\x10' + b'\xFF' * 184
            enhanced_log(f"[FMP4_CONVERT] Errore fallback TS completo: {fallback_error}", "WARNING", "proxy_ts")
            enhanced_log(f"🆘 [FMP4_CONVERT] Fallback di emergenza: {len(emergency_ts)} bytes", "WARNING", "proxy_ts")
            return emergency_ts
...
def is_subtitle_resource(url):
    ...
    return url_lower.endswith(subtitle_extensions) or any(marker in url_lower for marker in subtitle_markers)


def is_subtitle_media_tag(line):
    """Riconosce tag HLS EXT-X-MEDIA dedicati ai sottotitoli."""
    line_upper = (line or "").upper()
    return line_upper.startswith("#EXT-X-MEDIA") and "TYPE=SUBTITLES" in line_upper
...
def decrypt_ts_if_needed(ts_content, stream_id, headers, segment_url=None):
    ...
    enhanced_log(f"🔐 [DECRYPT_TS] === INIZIO DECRITTAZIONE per stream {stream_id} ===", "INFO", "AppCore")
    ...
    if non_ts_content_type:
        enhanced_log(f"[DECRYPT_TS] Risorsa non-TS rilevata ({non_ts_content_type}), decrittazione saltata", "INFO", "AppCore")
        return ts_content
    
    if len(ts_content) % 16 != 0:
        enhanced_log(f"[DECRYPT_TS] Lunghezza {len(ts_content)} non multipla di 16, segmento cifrato incompleto/non allineato", "WARNING", "AppCore")
        return make_fallback_ts_segment()

    # Log primo byte per debug
    enhanced_log(f"🔍 [DECRYPT_TS] Primo byte contenuto: 0x{ts_content[0]:02x}", "INFO", "AppCore")
    ...
    try:
        ...
        if not stream_info:
            enhanced_log(f"❌ [DECRYPT_TS] Stream info non trovato per {stream_id}", "ERROR", "AppCore")
            return ts_content
            
        enhanced_log(f"✅ [DECRYPT_TS] Stream info trovato per {stream_id}", "INFO", "AppCore")
        
        # Ottieni chiave AES
        aes_key = get_aes_key_for_stream(stream_id, headers, segment_url)
        if not aes_key:
            enhanced_log(f"❌ [DECRYPT_TS] Chiave AES non disponibile", "ERROR", "AppCore")
            return ts_content
            
        enhanced_log(f"✅ [DECRYPT_TS] Chiave AES ottenuta: {len(aes_key)} bytes", "INFO", "AppCore")
        ...
        if isinstance(iv, str):
            try:
                ...
                enhanced_log(f"🔢 [DECRYPT_TS] IV convertito da hex: {iv.hex()}", "DEBUG", "AppCore")
            except Exception as iv_error:
                iv = iv.encode('latin-1')
                enhanced_log(f"[DECRYPT_TS] IV non esadecimale, uso bytes latin-1: {iv_error}", "DEBUG", "AppCore")
                enhanced_log(f"🔢 [DECRYPT_TS] IV convertito da stringa: {iv.hex()}", "DEBUG", "AppCore")
        
        # Assicura che IV sia 16 bytes
        if len(iv) != 16:
            iv = (iv + b'\x00' * 16)[:16]
            enhanced_log(f"⚠️ [DECRYPT_TS] IV aggiustato a 16 bytes: {iv.hex()}", "WARNING", "AppCore")
        
        enhanced_log(f"🔢 [DECRYPT_TS] IV finale: {iv.hex()}", "DEBUG", "AppCore")
        enhanced_log(f"🔑 [DECRYPT_TS] Chiave: {aes_key.hex()}", "DEBUG", "AppCore")
        
        # Decrittazione
        enhanced_log(f"🔄 [DECRYPT_TS] Inizio decrittazione AES-128-CBC", "INFO", "AppCore")
        ...
        enhanced_log(f"✅ [DECRYPT_TS] Decrittazione completata: {len(decrypted)} bytes", "INFO", "AppCore")
        
        # Rimuovi padding PKCS7
        if len(decrypted) > 0:
            padding_length = decrypted[-1]
            enhanced_log(f"🔍 [DECRYPT_TS] Ultimo byte (padding): 0x{padding_length:02x}", "DEBUG", "AppCore")
            
            if 0 < padding_length <= 16:
                if all(b == padding_length for b in decrypted[-padding_length:]):
                    decrypted = decrypted[:-padding_length]
                    enhanced_log(f"✅ [DECRYPT_TS] Padding PKCS7 rimosso: {padding_length} bytes", "INFO", "AppCore")
                else:
                    enhanced_log(f"⚠️ [DECRYPT_TS] Padding non valido, mantengo contenuto originale", "WARNING", "AppCore")
            else:
                enhanced_log(f"⚠️ [DECRYPT_TS] Padding fuori range, nessuna rimozione", "WARNING", "AppCore")
        
        # Verifica sync byte finale
        if len(decrypted) > 0:
            enhanced_log(f"🔍 [DECRYPT_TS] Primo byte dopo decrittazione: 0x{decrypted[0]:02x}", "INFO", "AppCore")
            
            if is_valid_ts_payload(decrypted):
                enhanced_log(f"✅ [DECRYPT_TS] === DECRITTAZIONE RIUSCITA (sync byte 0x47) ===", "INFO", "AppCore")
                return decrypted
            else:
                enhanced_log(f"⚠️ [DECRYPT_TS] Sync byte non valido dopo decrittazione: 0x{decrypted[0]:02x}", "WARNING", "AppCore")
                # Prova senza rimuovere padding
                if AES_MODULE == "cryptography":
                    cipher = Cipher(algorithms.AES(aes_key), modes.CBC(iv), backend=default_backend())
                    decryptor = cipher.decryptor()
                    decrypted_no_padding = decryptor.update(ts_content) + decryptor.finalize()
                else:
                    cipher = AES_MODULE.new(aes_key, AES_MODULE.MODE_CBC, iv)
                    decrypted_no_padding = cipher.decrypt(ts_content)
                
                if len(decrypted_no_padding) > 0 and is_valid_ts_payload(decrypted_no_padding):
                    enhanced_log(f"✅ [DECRYPT_TS] === DECRITTAZIONE RIUSCITA (senza rimozione padding) ===", "INFO", "AppCore")
                    return decrypted_no_padding
                else:
                    enhanced_log(f"❌ [DECRYPT_TS] Decrittazione fallita, ritorno contenuto originale", "ERROR", "AppCore")
                    return make_fallback_ts_segment()
        else:
            enhanced_log(f"❌ [DECRYPT_TS] Contenuto decriptato vuoto", "ERROR", "AppCore")
            return make_fallback_ts_segment()
        
    except Exception as e:
        enhanced_log(f"❌ [DECRYPT_TS] === ERRORE DURANTE DECRITTAZIONE: {type(e).__name__}: {str(e)} ===", "ERROR", "AppCore")
        import traceback
        enhanced_log(f"🔍 [DECRYPT_TS] Stack trace: {traceback.format_exc()}", "ERROR", "AppCore")
        return make_fallback_ts_segment()
...
try:
    ...
    DLHD_SESSION = dlhd_extractor.session if hasattr(dlhd_extractor, 'session') else None
    ...
    class DLHDExtractor:
        ...
        def extract_stream(self, url):
            return None
    dlhd_extractor = DLHDExtractor()
    enhanced_log(f"⚠️ DLHD extractor non disponibile: {e}", "WARNING", "AppCore")
...
try:
    ...
    def is_vavoo_link(*args, **kwargs):
        return False
    enhanced_log(f"⚠️ Vavoo extractor non disponibile: {e}", "WARNING", "AppCore")
...
try:
    ...
    class vix_extractor:
        ...
        def extract(url):
            return None
    enhanced_log(f"⚠️ VixCloud extractor non disponibile: {e}", "WARNING", "AppCore")

# TVTap è ora gestito tramite WMS Manager e ServiceMonitor
def get_dlhd_session():
    if DLHD_AVAILABLE and dlhd_extractor and hasattr(dlhd_extractor, 'session'):
...
try:
    ...
    def is_sportonline_link(*args, **kwargs):
        return False
    enhanced_log(f"⚠️ Sportsonline extractor non disponibile: {e}", "WARNING", "AppCore")
...
try:
    ...
    def is_sport99_link(*args, **kwargs):
        return False
    enhanced_log(f"Sport99 extractor non disponibile: {e}", "WARNING", "AppCore")
...
try:
    ...
    class freeshot_extractor:
        ...
        def extract(url):
            return None
    enhanced_log(f"⚠️ Freeshot extractor non disponibile: {e}", "WARNING", "AppCore")
...
try:
    ...
    class MaxstreamExtractor:
        def extract(self, url, **kwargs):
            return None
    maxstream_extractor = MaxstreamExtractor()
    enhanced_log(f"⚠️ Maxstream extractor non disponibile: {e}", "WARNING", "AppCore")

# Mixdrop extractor
MIXDROP_DOMAINS = ('mixdrop.co', 'mixdrop.vip', 'm1xdrop.net', 'm1xdrop.bz', 'mixdrop.ch', 'mixdrop.ps', 'mixdrop.ag', 'mxcontent.net')
try:
    try:
    ...
    class MixdropExtractor:
        def extract(self, url, **kwargs):
            return None
    mixdrop_extractor = MixdropExtractor()
    enhanced_log(f"⚠️ Mixdrop extractor non disponibile: {e}", "WARNING", "AppCore")
...
enhanced_log(f"Timeout per le richieste impostato a {REQUEST_TIMEOUT} secondi.", "INFO", "AppCore")
...
try:
    ...
    MAX_KEEP_ALIVE_REQUESTS = int(os.environ.get('MAX_KEEP_ALIVE_REQUESTS', '100'))
    POOL_CONNECTIONS = int(os.environ.get('POOL_CONNECTIONS', '5'))
    POOL_MAXSIZE = int(os.environ.get('POOL_MAXSIZE', '10'))
    enhanced_log(f"Keep-Alive configurato: timeout={KEEP_ALIVE_TIMEOUT}s, max_requests={MAX_KEEP_ALIVE_REQUESTS}",
                 "INFO", "AppCore")
    ...
    POOL_CONNECTIONS = 5
    POOL_MAXSIZE = 10
enhanced_log(f"Keep-Alive disabilitato: timeout={KEEP_ALIVE_TIMEOUT}s, max_requests={MAX_KEEP_ALIVE_REQUESTS}", "INFO",
             "AppCore")
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt < max_attempts - 1:
                ...
                time.sleep(sleep_time * (attempt + 1))  # Retry più veloce per Freeshot
        except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as e:
            if attempt < max_attempts - 1:
                sleep_time = 0.1 if is_freeshot_url else 0.3
                time.sleep(sleep_time * (attempt + 1))  # Retry molto veloce per timeout
                continue
            if is_freeshot_url:
                enhanced_log(f"⚠️ [FREESHOT_TIMEOUT] Timeout dopo {max_attempts} tentativi: {url[-50:]}", "WARNING", "AppCore")
            else:
                enhanced_log(f"❌ Errore richiesta dopo {max_attempts} tentativi: {e}", "ERROR", "AppCore")
            raise
    ...
    try:
        ...
        for param in fragment.split('&'):
            if '=' in param:
                ...
                headers[key] = value
                enhanced_log(f"✅ [CUSTOM_HEADER] Estratto: {key} = {value[:50]}...", "DEBUG", "AppCore")
        
        return clean_url, headers
    except Exception as e:
        enhanced_log(f"❌ [CUSTOM_HEADER] Errore estrazione: {e}", "ERROR", "AppCore")
        return url, {}
...
def resolve_m3u8_link(url, headers=None, **kwargs):
    ...
    """
    enhanced_log(f"🚀 [RESOLVE_START] Inizio risoluzione URL: {url[:100]}...", "INFO", "AppCore")
    ...
    if custom_headers:
        enhanced_log(f"✅ [CUSTOM_HEADERS] Estratti {len(custom_headers)} headers custom", "INFO", "AppCore")
        url = clean_url  # Usa URL pulito senza fragment

    current_headers = headers.copy() if headers else {}
    # Aggiungi headers custom estratti
    current_headers.update(custom_headers)
    enhanced_log(f"🔍 [RESOLVE_HEADERS] Headers iniziali: {len(current_headers)} elementi", "DEBUG", "AppCore")

    # 1. Estrazione header dall'URL
    enhanced_log("📋 [RESOLVE_STEP1] Inizio estrazione header dall'URL", "INFO", "AppCore")
    clean_url = url
    extracted_headers = {}

    if '&h_' in url or '%26h_' in url:
        enhanced_log("🔍 [RESOLVE_HEADERS] Rilevati parametri header nell'URL", "INFO", "AppCore")
        temp_url = url

        if 'vavoo.to' in temp_url.lower() and '%26' in temp_url:
            temp_url = temp_url.replace('%26', '&')
            enhanced_log("🔄 [RESOLVE_HEADERS] Sostituito %26 con & per Vavoo", "DEBUG", "AppCore")

        if '%26h_' in temp_url:
            temp_url = unquote(unquote(temp_url))
            enhanced_log("🔄 [RESOLVE_HEADERS] Doppio unquote applicato", "DEBUG", "AppCore")

        url_parts = temp_url.split('&h_', 1)
        clean_url = url_parts[0]
        header_params = '&h_' + url_parts[1]
        enhanced_log(f"🔍 [RESOLVE_HEADERS] URL pulito: {clean_url[:50]}...", "DEBUG", "AppCore")
        enhanced_log(f"🔍 [RESOLVE_HEADERS] Parametri header: {header_params[:100]}...", "DEBUG", "AppCore")
        ...
        for param in header_params.split('&'):
            if param.startswith('h_'):
                try:
                    ...
                    enhanced_log(f"❌ [RESOLVE_HEADERS] Errore estrazione {param}: {e}", "ERROR", "AppCore")

    final_headers = {**current_headers, **extracted_headers}
    enhanced_log(f"✅ [RESOLVE_STEP1] Headers finali: {len(final_headers)} elementi", "INFO", "AppCore")

    # 2. Controllo tipo URL con extractor separati
    enhanced_log("📋 [RESOLVE_STEP2] Controllo tipo URL", "INFO", "AppCore")

    # Controllo Sport99 / CDNLiveTV - dominio specifico prima dei matcher generici
    if SPORT99_AVAILABLE and is_sport99_link(clean_url):
        enhanced_log(f"[RESOLVE_SPORT99] Rilevato link Sport99/CDNLiveTV: {clean_url[:50]}...", "INFO", "AppCore")
        try:
            resolved_sport99 = extract_sport99(clean_url, final_headers)
            if resolved_sport99 and resolved_sport99.get("resolved_url"):
                enhanced_log("[RESOLVE_SPORT99] Sport99 risolto con successo", "INFO", "AppCore")
                sport99_headers = resolved_sport99.get("headers") or resolved_sport99.get("request_headers") or {}
                ...
                enhanced_log("[RESOLVE_SPORT99] Sport99 resolver ha restituito None", "WARNING", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"[RESOLVE_SPORT99] Errore Sport99 resolver: {e}", "ERROR", "AppCore")
            return {"resolved_url": clean_url, "headers": final_headers}

    # Controllo PowerSet (LiveTV), dopo i domini piu specifici
    if LIVETV_AVAILABLE and is_powerset_domain(clean_url):
        enhanced_log(f"✅ [RESOLVE_POWERSET] Rilevato dominio powerset: {clean_url[:50]}...", "INFO", "AppCore")
        try:
            ...
            if resolved_powerset and resolved_powerset.get("resolved_url"):
                enhanced_log("✅ [RESOLVE_POWERSET] PowerSet risolto con successo", "INFO", "AppCore")
                # Combina gli headers esistenti con quelli del resolver
                combined_headers = {**final_headers, **resolved_powerset.get("headers", {})}
                return {
                    "resolved_url": resolved_powerset["resolved_url"],
                    "headers": combined_headers,
                    "mediaflow_endpoint": resolved_powerset.get("mediaflow_endpoint"),
                    "query_params": resolved_powerset.get("query_params", {})
                }
            else:
                enhanced_log("⚠️ [RESOLVE_POWERSET] PowerSet resolver ha restituito None", "WARNING", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"❌ [RESOLVE_POWERSET] Errore PowerSet resolver: {e}", "ERROR", "AppCore")
            return {"resolved_url": clean_url, "headers": final_headers}

    # Controllo DaddyLive/DLHD con extractor separato
    if DLHD_AVAILABLE and dlhd_extractor.is_daddylive_link(clean_url):
        enhanced_log(f"✅ [RESOLVE_DLHD] Rilevato link DaddyLive/DLHD: {clean_url[:50]}...", "INFO", "AppCore")
        enhanced_log("🚀 [DLHD_FLOW] === INIZIO FLUSSO ESTRAZIONE DADDYLIVE ===", "INFO", "AppCore")
        try:
            resolved_dlhd = dlhd_extractor.extract_stream(clean_url)
            if resolved_dlhd and resolved_dlhd.get("destination_url"):
                enhanced_log("✅ [DLHD_FLOW] === FLUSSO COMPLETATO CON SUCCESSO ===", "INFO", "AppCore")
                enhanced_log(f"🎯 [DLHD_FLOW] URL finale: {resolved_dlhd['destination_url']}", "INFO", "AppCore")
                
                # Combina gli headers finali con quelli del resolver
                combined_headers = {**final_headers, **resolved_dlhd.get("request_headers", {})}
                
                # Log headers per debug
                enhanced_log(f"📤 [DLHD_FLOW] Headers combinati: {list(combined_headers.keys())}", "DEBUG", "AppCore")
                
                result = {"resolved_url": resolved_dlhd["destination_url"], "headers": combined_headers}
                if resolved_dlhd.get("captured_manifest"):
                    result["m3u8_content"] = resolved_dlhd["captured_manifest"]
                return result
            else:
                enhanced_log("❌ [DLHD_FLOW] === FLUSSO FALLITO - NESSUN URL ===", "ERROR", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"❌ [DLHD_FLOW] === FLUSSO FALLITO - ERRORE: {e} ===", "ERROR", "AppCore")
            import traceback
            enhanced_log(f"🔍 [DLHD_FLOW] Traceback: {traceback.format_exc()}", "DEBUG", "AppCore")
            return {"resolved_url": clean_url, "headers": final_headers}

    # Controllo VixCloud con extractor separato
    if VIX_AVAILABLE and any(vix_domain in clean_url.lower() for vix_domain in ['vix', 'vixcloud', 'vixsrc']):
        enhanced_log(f"✅ [RESOLVE_VIX] Rilevato link VixCloud: {clean_url[:50]}...", "INFO", "AppCore")
        try:
            resolved_vix = vix_extractor.extract(clean_url)
            if resolved_vix and (resolved_vix.get("resolved_url") or resolved_vix.get("m3u8_content")):
                enhanced_log("✅ [RESOLVE_VIX] VixCloud risolto con successo", "INFO", "AppCore")
                # CORREZIONE: Restituisce l'intero dizionario dal resolver Vix,
                # non solo una parte, per preservare 'm3u8_content'.
                resolved_vix["headers"] = {**final_headers, **resolved_vix.get("headers", {})}
                return resolved_vix
            else:
                enhanced_log("⚠️ [RESOLVE_VIX] VixCloud resolver ha restituito None", "WARNING", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"❌ [RESOLVE_VIX] Errore VixCloud resolver: {e}", "ERROR", "AppCore")
            return {"resolved_url": clean_url, "headers": final_headers}

    # Controllo Vavoo con extractor separato
    if VAVOO_AVAILABLE and is_vavoo_link(clean_url):
        enhanced_log(f"✅ [RESOLVE_VAVOO] Rilevato link Vavoo: {clean_url[:50]}...", "INFO", "AppCore")
        try:
            resolved_vavoo = vavoo_extractor.extract(clean_url, final_headers)
            if resolved_vavoo and resolved_vavoo.get("resolved_url"):
                enhanced_log("✅ [RESOLVE_VAVOO] Vavoo risolto con successo", "INFO", "AppCore")
                return resolved_vavoo
            else:
                enhanced_log("⚠️ [RESOLVE_VAVOO] Vavoo resolver ha restituito None", "WARNING", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"❌ [RESOLVE_VAVOO] Errore Vavoo resolver: {e}", "ERROR", "AppCore")
            # FALLBACK: Se Vavoo fallisce, prova a restituire un M3U8 di errore più user-friendly
            if "timeout" in str(e).lower() or "connection" in str(e).lower():
                enhanced_log("🔄 [RESOLVE_VAVOO] Timeout/connessione - restituisco fallback", "WARNING", "AppCore")
                return {
                    "resolved_url": None,
    ...
    if SPORTONLINE_AVAILABLE and is_sportonline_link(clean_url):
        enhanced_log(f"✅ [RESOLVE_SPORTONLINE] Rilevato link Sportsonline: {clean_url[:50]}...", "INFO", "AppCore")
        try:
            resolved_sport = extract_sportonline(clean_url)
            if resolved_sport and resolved_sport.get("resolved_url"):
                enhanced_log("✅ [RESOLVE_SPORTONLINE] Sportsonline risolto con successo", "INFO", "AppCore")
                combined_headers = {**final_headers, **resolved_sport.get("headers", {})}
                return {"resolved_url": resolved_sport["resolved_url"], "headers": combined_headers}
            else:
                enhanced_log("⚠️ [RESOLVE_SPORTONLINE] Sportsonline resolver ha restituito None", "WARNING", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"❌ [RESOLVE_SPORTONLINE] Errore Sportsonline resolver: {e}", "ERROR", "AppCore")
            return {"resolved_url": clean_url, "headers": final_headers}

    # Controllo Freeshot
    if FREESHOT_AVAILABLE and is_freeshot_link(clean_url):
        enhanced_log(f"✅ [RESOLVE_FREESHOT] Rilevato link Freeshot: {clean_url[:50]}...", "INFO", "AppCore")
        try:
            resolved_freeshot = freeshot_extractor.extract(clean_url)
            if resolved_freeshot and resolved_freeshot.get("resolved_url"):
                enhanced_log("✅ [RESOLVE_FREESHOT] Freeshot risolto con successo", "INFO", "AppCore")
                combined_headers = {**final_headers, **resolved_freeshot.get("headers", {})}
                return {"resolved_url": resolved_freeshot["resolved_url"], "headers": combined_headers}
            else:
                enhanced_log("⚠️ [RESOLVE_FREESHOT] Freeshot resolver ha restituito None", "WARNING", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"❌ [RESOLVE_FREESHOT] Errore Freeshot resolver: {e}", "ERROR", "AppCore")
            return {"resolved_url": clean_url, "headers": final_headers}

    # Controllo Maxstream
    if MAXSTREAM_AVAILABLE and is_maxstream_link(clean_url):
        enhanced_log(f"✅ [RESOLVE_MAXSTREAM] Rilevato link Maxstream: {clean_url[:50]}...", "INFO", "AppCore")
        try:
            # Estrai parametri season/episode se presenti
            season = kwargs.get('season')
            episode = kwargs.get('episode')
            resolved_maxstream = maxstream_extractor.extract(clean_url, season=season, episode=episode)
            if resolved_maxstream and resolved_maxstream.get("resolved_url"):
                enhanced_log("✅ [RESOLVE_MAXSTREAM] Maxstream risolto con successo", "INFO", "AppCore")
                combined_headers = {**final_headers, **resolved_maxstream.get("headers", {})}
                return {"resolved_url": resolved_maxstream["resolved_url"], "headers": combined_headers}
            else:
                enhanced_log("⚠️ [RESOLVE_MAXSTREAM] Maxstream resolver ha restituito None", "WARNING", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"❌ [RESOLVE_MAXSTREAM] Errore Maxstream resolver: {e}", "ERROR", "AppCore")
            return {"resolved_url": clean_url, "headers": final_headers}

    # Controllo Mixdrop
    if MIXDROP_AVAILABLE and (is_mixdrop_link(clean_url) or any(domain in clean_url.lower() for domain in MIXDROP_DOMAINS)):
        enhanced_log(f"✅ [RESOLVE_MIXDROP] Rilevato link Mixdrop: {clean_url[:50]}...", "INFO", "AppCore")
        try:
            resolved_mixdrop = mixdrop_extractor.extract(clean_url)
            mixdrop_url = resolved_mixdrop.get("resolved_url") or resolved_mixdrop.get("destination_url") if resolved_mixdrop else None
            mixdrop_headers = resolved_mixdrop.get("headers") or resolved_mixdrop.get("request_headers") if resolved_mixdrop else {}
            if resolved_mixdrop and mixdrop_url:
                enhanced_log("✅ [RESOLVE_MIXDROP] Mixdrop risolto con successo", "INFO", "AppCore")
                combined_headers = {**final_headers, **mixdrop_headers}
                return {"resolved_url": mixdrop_url, "headers": combined_headers}
            else:
                enhanced_log("⚠️ [RESOLVE_MIXDROP] Mixdrop resolver ha restituito None", "WARNING", "AppCore")
                return {"resolved_url": clean_url, "headers": final_headers}
        except Exception as e:
            enhanced_log(f"❌ [RESOLVE_MIXDROP] Errore Mixdrop resolver: {e}", "ERROR", "AppCore")
            return {"resolved_url": clean_url, "headers": final_headers}

    # Controllo TVTap diretto
    if TVTAP_AVAILABLE and any(pattern in clean_url.lower() for pattern in ['tvtap', 'rocktalk.net', 'taptube.net', 'authsign=', 'stream.mardio.link']):
                enhanced_log(f"✅ [RESOLVE_TVTAP] Rilevato link TVTap: {clean_url[:50]}...", "INFO", "AppCore")
                try:
                    ...
                    enhanced_log(f"❌ [RESOLVE_TVTAP] Errore TVTap resolver: {e}", "ERROR", "AppCore")
                    return {"resolved_url": clean_url, "headers": final_headers}

    # URL generico - passthrough
    enhanced_log("🔄 [RESOLVE_PASSTHROUGH] URL generico, passthrough", "INFO", "AppCore")
    return {"resolved_url": clean_url, "headers": final_headers}
...
def get_dynamic_timeout(url, base_timeout=REQUEST_TIMEOUT):
    ...
    # CORREZIONE: Timeout ridotti per migliorare la reattività del cambio canale
    if '.ts' in url_lower:
        ...
        if any(d in url_lower for d in ['vavoo', 'shouurvki7jtfax', 'ngolpdkyoctjcddxshli469r']):
...
def _get_vavoo_prefetch_entry(m3u_url, wait_timeout=0, consume=True):
    ...
    if entry.get('in_progress') and wait_timeout:
        ...
        if event:
            enhanced_log(f"[VAVOO_PREFETCH] Attendo prefetch per {stream_id} fino a {wait_timeout}s", "INFO", "AppCore")
    ...
    if entry.get('in_progress'):
        enhanced_log(f"[VAVOO_PREFETCH] Prefetch ancora in corso per {stream_id}", "DEBUG", "AppCore")
        return None
    ...
    enhanced_log(f"⚡ [VAVOO_PREFETCH] M3U8 da prefetch per stream {stream_id}", "INFO", "AppCore")
    if consume:
...
def _clear_vavoo_resolved_url_cache(reason=""):
    ...
    try:
        ...
        if isinstance(cache, dict):
            cache.clear()
            enhanced_log(f"[VAVOO] Cache URL risolti pulita{': ' + reason if reason else ''}", "INFO", "AppCore")
        elif hasattr(vavoo_extractor, "clear_cache"):
            vavoo_extractor.clear_cache()
            enhanced_log(f"[VAVOO] Cache extractor pulita{': ' + reason if reason else ''}", "INFO", "AppCore")
    except Exception as exc:
        enhanced_log(f"[VAVOO] Errore pulizia cache URL: {exc}", "DEBUG", "AppCore")
...
def prefetch_vavoo_m3u8(m3u_url, headers=None):
    ...
    if existing and time.time() - existing.get('timestamp', 0) < 20:
        enhanced_log(f"⚡ [VAVOO_PREFETCH] Prefetch già disponibile/in corso per {stream_id}", "DEBUG", "AppCore")
        return True

    event = threading.Event()
    VAVOO_M3U8_PREFETCH_CACHE[stream_id] = {'in_progress': True, 'timestamp': time.time(), 'event': event}

    def _worker():
        try:
            enhanced_log(f"⚡ [VAVOO_PREFETCH] Avvio prefetch per {stream_id}", "INFO", "AppCore")
            ...
            if final_url:
                ...
                enhanced_log(f"✅ [VAVOO_PREFETCH] M3U8 pronto per {stream_id}: {len(m3u_content)} caratteri", "INFO", "AppCore")
            else:
                VAVOO_M3U8_PREFETCH_CACHE.pop(stream_id, None)
                enhanced_log(f"⚠️ [VAVOO_PREFETCH] Contenuto non valido per {stream_id}", "WARNING", "AppCore")
        except Exception as exc:
            VAVOO_M3U8_PREFETCH_CACHE.pop(stream_id, None)
            _clear_vavoo_resolved_url_cache("prefetch fallito")
            enhanced_log(f"⚠️ [VAVOO_PREFETCH] Fallito per {stream_id}: {exc}", "WARNING", "AppCore")
        finally:
            event.set()
...
def wait_vavoo_prefetch(m3u_url, timeout=0):
    ...
    return _get_vavoo_prefetch_entry(m3u_url, wait_timeout=timeout, consume=False)
...
def _fetch_vavoo_playlist(url, headers, timeout):
    ...
    enhanced_log("[VAVOO] Download playlist con sessione fresh", "INFO", "AppCore")
...
def get_proxy_for_url(url):
    ...
    if is_vavoo:
        ...
        if proxy_pool:
            ...
            enhanced_log("[VAVOO_PROXY] Uso proxy per CDN/resolve Vavoo", "INFO", "AppCore")
...
def refresh_freeshot_token(channel_name, old_token):
    ...
    try:
        ...
        enhanced_log(f"🔄 [FREESHOT_REFRESH] Refresh token per canale: {channel_name}", "INFO", "AppCore")
        ...
        enhanced_log(f"🔄 [FREESHOT_REFRESH] Tentativo refresh: {freeshot_url}", "DEBUG", "AppCore")
        ...
        if fresh_result and fresh_result.get('resolved_url'):
            ...
            if token_match:
                ...
                enhanced_log(f"✅ [FREESHOT_REFRESH] Nuovo token ottenuto: {new_token[:20]}...", "INFO", "AppCore")
                return new_token
            else:
                enhanced_log(f"⚠️ [FREESHOT_REFRESH] Token non trovato nell'URL", "WARNING", "AppCore")
                return None
        else:
            enhanced_log(f"❌ [FREESHOT_REFRESH] Extractor fallito", "ERROR", "AppCore")
            return None
            
    except Exception as e:
        enhanced_log(f"❌ [FREESHOT_REFRESH] Errore refresh token: {e}", "ERROR", "AppCore")
        return None
...
def get_stream_id_from_url(url):
    ...
    if 'daddyhd.com' in url or 'daddylive' in url:
        ...
        if channel_match:
            ...
            stream_id = hashlib.sha256(f"daddy_{channel_id}".encode()).hexdigest()[:12]
            enhanced_log(f"🆔 [STREAM_ID] Stream ID deterministico per canale DaddyLive {channel_id}: {stream_id}", "INFO", "AppCore")
            return stream_id

    # Per VIX, usa ID stabili invece dell'URL completo: token/rendition cambiano spesso.
    if 'vixsrc.to/tv/' in url or 'vixcloud.co/tv/' in url:
        ...
        if tv_match:
            ...
            stream_id = hashlib.sha256(f"vix_tv_{tv_key}".encode()).hexdigest()[:12]
            enhanced_log(f"ðŸ†” [STREAM_ID] Stream ID deterministico per VIX tv {tv_key}: {stream_id}", "INFO", "AppCore")
            return stream_id
    ...
    if 'vixsrc.to/playlist/' in url or 'vixcloud.co/playlist/' in url:
        ...
        if playlist_match:
            ...
            stream_id = hashlib.sha256(f"vix_{playlist_id}".encode()).hexdigest()[:12]
            enhanced_log(f"🆔 [STREAM_ID] Stream ID deterministico per VIX playlist {playlist_id}: {stream_id}", "INFO", "AppCore")
            return stream_id
    ...
    if 'lovecdn.ru' in url:
        ...
        if channel_match:
            ...
            stream_id = hashlib.sha256(f"freeshot_{channel_name}".encode()).hexdigest()[:12]
            enhanced_log(f"🆔 [STREAM_ID] Stream ID deterministico per Freeshot {channel_name}: {stream_id}", "INFO", "AppCore")
            return stream_id
    ...
    if 'vavoo' in url.lower():
        ...
        if channel_match:
            channel_id = channel_match.group(1)
            stream_id = hashlib.sha256(f"vavoo_{channel_id}".encode()).hexdigest()[:12]
            enhanced_log(f"🆔 [STREAM_ID] Stream ID deterministico per Vavoo {channel_id}: {stream_id}", "INFO", "AppCore")
            return stream_id

    # Per altri URL, usa l'URL stesso come base (meno ideale, ma comunque stabile)
    stream_id = hashlib.sha256(url.encode()).hexdigest()[:12]
    enhanced_log(f"🆔 [STREAM_ID] Stream ID deterministico generico: {stream_id}", "INFO", "AppCore")
    return stream_id
...
def replace_key_uri(line, headers_query):
    ...
    if match:
        ...
        proxied_key_url = f"http://127.0.0.1:7860/proxy/key?url={quote(key_url)}&{headers_query}"
...
class RouteRegistry:
    ...
    def dispatch(self, name, *args, **kwargs):
        enhanced_log(f"🔍 [DEBUG] dispatch {name}: args={args}, kwargs={kwargs}", "DEBUG", "AppCore")

        if name in self.routes:
            return self.routes[name](**kwargs)  # Assicurati di passare **kwargs
...
def proxy_mpd(request=None, **kwargs):
    ...
    enhanced_log(f"⚠️ [MPD] Enigma2 non supporta DASH: {mpd_url[:100]}...", "WARNING", "AppCore")
...
def proxy_m3u(request=None, **kwargs):
    ...
    try:
        ...
        if custom_headers:
            enhanced_log(f"📤 [PROXY_M3U] Headers custom estratti: {list(custom_headers.keys())}", "DEBUG", "AppCore")
        
        # ✅ ESTRAI PARAMETRO dlhd_masked dalla richiesta (verrà aggiornato dopo resolve)
        dlhd_masked_param = kwargs.get('dlhd_masked', '0') == '1'
        enhanced_log(f"🎭 [DLHD_MASKED] Parametro dlhd_masked iniziale: {dlhd_masked_param}", "DEBUG", "AppCore")
        ...
        if is_vavoo_request:
            ...
            if cached_m3u8:
                ...
                if cached_content and cached_m3u8.get('in_progress') and cached_age < 60:
                    enhanced_log(f"[VAVOO_M3U8_CACHE] Refresh gia' in corso, riuso ultima playlist valida ({int(cached_age)}s)", "INFO", "AppCore")
                    return {
                ...
                if cached_content and cached_age < 6:
                    enhanced_log(f"[VAVOO_M3U8_CACHE] Playlist recente da cache ({int(cached_age)}s)", "INFO", "AppCore")
                    return {
                        'content': cached_content,
                        'status': 200,
                        'content_type': 'application/vnd.apple.mpegurl'
            ...
            VAVOO_FINAL_M3U8_CACHE[current_stream_id] = {
                ...
                'timestamp': cached_m3u8.get('timestamp', 0) if cached_m3u8 else 0,
                'in_progress': True,
                'started': time.time(),
            }
        prefetched_result = _get_vavoo_prefetch_entry(m3u_url, wait_timeout=2 if is_vavoo_request else 0)
        ...
        if not is_same_channel:
            ...
            clear_stream_cache()
            enhanced_log(f"🧹 [CACHE_CLEAR] Cache pulite per cambio canale. Nuovo stream_id: {current_stream_id}", "INFO", "AppCore")
        else:
            # ✅ NUOVO: Se è lo stesso canale, riusa la cache ma aggiorna timestamp
            enhanced_log(f"♻️  [CACHE_REUSE] Stesso canale (stream_id: {current_stream_id}), cache riutilizzata", "INFO", "AppCore")
                ...
                try:
                    ...
                    if auth_header.startswith('Bearer '):
                        ...
                        try:
                            ...
                            # Refresh solo se mancano meno di 2 minuti alla scadenza
                            if remaining_time < 120:
                                enhanced_log(f"🔄 [TOKEN_REFRESH] Token scade tra {int(remaining_time)}s, refresh necessario", "INFO", "AppCore")

                                # Invalida cache DLHD per questo canale
                                if DLHD_AVAILABLE and dlhd_extractor:
                                    try:
                                        channel_match = re.search(r'[?&]id=(\d+)', m3u_url)
                                        if channel_match:
                                            channel_id = channel_match.group(1)
                                            if hasattr(dlhd_extractor, 'cache') and channel_id in dlhd_extractor.cache:
                                                del dlhd_extractor.cache[channel_id]
                                                enhanced_log(f"🗑️ [DLHD_CACHE] Cache invalidata per canale {channel_id}", "INFO", "AppCore")
                                    except Exception as cache_error:
                                        enhanced_log(f"[DLHD_CACHE] Errore invalidazione cache: {cache_error}", "DEBUG", "AppCore")
                                
                                # Rimuovi stream per forzare nuovo download
                                del STREAM_KEY_INFO[current_stream_id]
                                keys_to_remove = [k for k in AES_KEY_CACHE.keys() if k.startswith(current_stream_id)]
                                for k in keys_to_remove:
                                    del AES_KEY_CACHE[k]
                                is_same_channel = False  # Forza nuovo download
                                enhanced_log(f"♻️  [CACHE_INVALIDATED] Cache invalidata per refresh token", "INFO", "AppCore")
                            else:
                                enhanced_log(f"✅ [TOKEN_VALID] Token valido per altri {int(remaining_time)}s", "DEBUG", "AppCore")
                        except ImportError:
                            enhanced_log("⚠️ [TOKEN_CHECK] PyJWT non disponibile, uso fallback temporale", "WARNING", "AppCore")
                            # Fallback: refresh ogni 4 minuti se PyJWT non disponibile
                            last_used = stream_info.get('last_used', 0)
                            if time.time() - last_used > 240:
                                enhanced_log(f"🔄 [FALLBACK_REFRESH] Refresh fallback dopo 4 minuti", "INFO", "AppCore")
                                del STREAM_KEY_INFO[current_stream_id]
                                is_same_channel = False
                        except Exception as e:
                            enhanced_log(f"⚠️ [TOKEN_CHECK] Errore verifica token: {e}", "WARNING", "AppCore")
                except Exception as e:
                    enhanced_log(f"⚠️ [TOKEN_CHECK] Errore controllo token: {e}", "WARNING", "AppCore")
        
        enhanced_log("🔍 [DEBUG] Chiamata resolve_m3u8_link direttamente", "DEBUG", "AppCore")
        if prefetched_result:
            enhanced_log("[VAVOO_PREFETCH] Uso risultato pre-risolto", "INFO", "AppCore")
            result = prefetched_result
            if custom_headers:
                result["headers"] = {**result.get("headers", {}), **custom_headers}
        else:
            result = resolve_m3u8_link(m3u_url, custom_headers)
        enhanced_log(f"🔍 [DEBUG] Risultato resolve: {result}", "DEBUG", "AppCore")
        ...
        dlhd_masked = kwargs.get('dlhd_masked', '0') == '1' or is_dlhd_domain or is_daddy_domain(m3u_url)
        enhanced_log(f"🎭 [DLHD_MASKED] Parametro dlhd_masked: {dlhd_masked} (auto-rilevato: {is_dlhd_domain})", "INFO", "AppCore")
        ...
        # h_Referer arrivato dalla playlist master. VIX lo usa per key/segmenti.
        ...
        if final_url and is_direct_media_url(final_url):
            ...
            enhanced_log(f"🎬 [DIRECT_MEDIA] URL diretto risolto, genero playlist wrapper: {final_url[:80]}...", "INFO", "AppCore")
            return {
                'content': b'',
                'status': 302,
        ...
        if not m3u_content:
            ...
            timeout_to_use = 6 if 'kiko2.ru' in final_url else get_dynamic_timeout(final_url)
            enhanced_log(f"🕐 [M3U8_TIMEOUT] Timeout ottimizzato: {timeout_to_use}s", "DEBUG", "AppCore")
            
            # Usa proxy per Mixdrop
            proxy_config = get_proxy_for_url(final_url)
            proxy_url = proxy_config['http'] if proxy_config else None
            
            if VAVOO_AVAILABLE and is_vavoo_link(m3u_url) and _is_vavoo_cdn_url(final_url):
                m3u_response = _fetch_vavoo_playlist(final_url, current_headers_for_proxy, timeout_to_use)
            else:
            ...
            if (
                ...
                enhanced_log("[VAVOO] 403 su URL CDN risolto, pulisco cache e ritento una volta", "WARNING", "AppCore")
            ...
            if (
                ...
                enhanced_log("[VAVOO] 403 persistente, provo varianti header CDN", "WARNING", "AppCore")
                for variant_headers in _vavoo_playlist_header_variants(current_headers_for_proxy):
                    if _is_vavoo_cdn_url(final_url):
                        m3u_response = _fetch_vavoo_playlist(final_url, variant_headers, timeout_to_use)
                    else:
                        m3u_response = make_persistent_request(
                            final_url,
                    ...
                    if m3u_response.status_code != 403:
                        ...
                        enhanced_log(f"[VAVOO] Playlist CDN accettata con headers: {list(variant_headers.keys())}", "INFO", "AppCore")
                        break
            if not m3u_content:
            ...
            if not m3u_content.strip().startswith('#EXTM3U'):
                enhanced_log("❌ [M3U8_VALIDATION] Contenuto non è un M3U8 valido (probabilmente HTML)", "ERROR", "AppCore")
                enhanced_log(f"📄 [M3U8_VALIDATION] Primi 200 caratteri: {m3u_content[:200]}", "DEBUG", "AppCore")
                return {
            ...
            enhanced_log("ℹ️ [proxy_m3u] Utilizzo contenuto M3U8 pre-processato dal resolver.", "INFO", "AppCore")

        enhanced_log(f"📄 [M3U8_CONTENT] Contenuto M3U8 ricevuto ({len(m3u_content)} caratteri):", "DEBUG", "AppCore")
        enhanced_log(f"📄 [M3U8_CONTENT] Prime 500 caratteri: {m3u_content[:500]}", "DEBUG", "AppCore")
        ...
        extract_key_info_from_m3u8(m3u_content.splitlines(), stream_id, base_url, current_headers_for_proxy, result)
        ...
        if stream_id not in STREAM_KEY_INFO:
            ...
            enhanced_log(f"💾 [STREAM_INFO] Salvate info base per stream {stream_id}", "DEBUG", "AppCore")
        else:
            # ✅ AGGIORNAMENTO CRITICO: Aggiorna headers se lo stream esiste già
            # Questo è fondamentale per i segmenti TS che devono ricevere gli headers corretti
            ...
            enhanced_log(f"🔄 [STREAM_INFO] Aggiornati headers per stream esistente {stream_id}", "DEBUG", "AppCore")
            enhanced_log(f"📝 [STREAM_INFO] Headers salvati: {list(existing_headers.keys())}", "DEBUG", "AppCore")
        
        # Pulisci automaticamente segmenti scaduti più frequentemente
        if random.randint(1, 5) == 1:  # 20% di probabilità per pulizia più frequente
            ...
            if line.startswith("#EXT-X-KEY") and 'AES-128' in line:
                ...
                if is_daddy_domain(final_url) or is_daddy_domain(m3u_url):
                    ...
                    if uri_match:
                        ...
                        proxy_key_url = f"http://127.0.0.1:7860/proxy/key?url={quote(key_url)}&{headers_query}"
                        aes_key_line = line.replace(uri_match.group(1), proxy_key_url)
                        enhanced_log(f"🔑 [DLHD_KEY] Chiave AES mantenuta nel M3U8: {key_url[-30:]}", "INFO", "AppCore")
                continue
                ...
                if uri_match:
                    ...
                    enhanced_log(f"📥 [INIT_MAP] Init segment URL: {init_url}", "INFO", "AppCore")
                    ...
                    if STREAM_KEY_INFO[stream_id].get('is_freeshot', False):
                        ...
                        if headers_query:
                            proxy_init_url = f"http://127.0.0.1:7860/proxy/ts?url={quote(init_url)}&fmp4=1&stream_id={stream_id}&{headers_query}"
                        else:
                            proxy_init_url = f"http://127.0.0.1:7860/proxy/ts?url={quote(init_url)}&fmp4=1&stream_id={stream_id}"
                        
                        # Sostituisci l'URI originale con il proxy
                        line = line.replace(uri_match.group(1), proxy_init_url)
                        enhanced_log(f"✅ [INIT_MAP] EXT-X-MAP mantenuto con proxy per Freeshot", "INFO", "AppCore")
                        modified_m3u8.append(line)
                        continue
                    else:
                        # Per altri provider, scarica e salva init segment (comportamento originale)
                        try:
                            ...
                            if len(init_content) >= 8:
                                ...
                                enhanced_log(f"✅ [INIT_DOWNLOAD] Init segment salvato: {len(init_content)} bytes", "INFO", "AppCore")
                            
                            # Rimuovi EXT-X-MAP per compatibilità
                            enhanced_log(f"⚠️ [INIT_SKIP] Rimosso EXT-X-MAP per compatibilità Enigma2", "INFO", "AppCore")
                            continue
                            
                        except Exception as e:
                            enhanced_log(f"❌ [INIT_DOWNLOAD] Errore download init: {e}", "ERROR", "AppCore")
                            # Rimuovi EXT-X-MAP anche in caso di errore
                            continue
                else:
                    enhanced_log(f"⚠️ [INIT_SKIP] EXT-X-MAP senza URI valido", "WARNING", "AppCore")
                    continue
            elif line.startswith("#EXT-X-MEDIA") and 'URI="' in line:
                if is_subtitle_media_tag(line):
                    enhanced_log(f"[SUBTITLE_SKIP] Traccia sottotitoli rimossa dall'M3U8 per evitare richieste VTT via TS", "INFO", "AppCore")
                    continue
                ...
                if uri_match:
                    ...
                    if is_subtitle_resource(media_url):
                        enhanced_log(f"[SUBTITLE_SKIP] Playlist sottotitoli rimossa: {media_url[-60:]}", "INFO", "AppCore")
                        continue
                    if headers_query:
                        proxy_media_url = f"http://127.0.0.1:7860/proxy/m3u?url={quote(media_url)}&{headers_query}"
                    else:
                        proxy_media_url = f"http://127.0.0.1:7860/proxy/m3u?url={quote(media_url)}"
                ...
                if is_subtitle_resource(segment_url):
                    enhanced_log(f"[SUBTITLE_SKIP] Segmento sottotitoli scartato: {line}", "INFO", "AppCore")
                    continue
                
                # ✅ CORREZIONE CRITICA: Validazione migliorata per segmenti DLHD
                is_dlhd_domain = is_daddy_domain(segment_url_lower) or dlhd_masked
                
                if is_dlhd_domain:
                    # Per DLHD, mantieni TUTTI i segmenti da domini DLHD (anche quelli codificati)
                    # Scarta solo immagini, font, CSS/JS reali
                    if any(ext in segment_url_lower for ext in ['.png', '.jpg', '.gif', '.svg', '.xml', '.json', '.woff', '.ttf', '.ico', '.css', '.js']):
                        enhanced_log(f"⚠️ [SEGMENT_SKIP] Segmento non-video scartato (DLHD): {line}", "WARNING", "AppCore")
                        continue
                    # Accetta TUTTI gli altri segmenti da domini DLHD (inclusi quelli codificati)
                    enhanced_log(f"✅ [DLHD_SEGMENT] Segmento DLHD accettato: {line[:50]}...", "DEBUG", "AppCore")
                else:
                    # Per altri provider, scarta .html/.css/.js/.txt normalmente
                    if any(ext in segment_url_lower for ext in ['.js', '.html', '.txt', '.css', '.json']):
                        enhanced_log(f"⚠️ [SEGMENT_SKIP] Segmento non-video scartato: {line}", "WARNING", "AppCore")
                        continue
                ...
                if '.m3u8' in segment_url_lower or 'playlist' in segment_url_lower:
                    ...
                    if headers_query:
                        proxy_url = f"http://127.0.0.1:7860/proxy/m3u?url={quote(segment_url)}&{headers_query}"
                    else:
                        proxy_url = f"http://127.0.0.1:7860/proxy/m3u?url={quote(segment_url)}"
                    ...
                    enhanced_log(f"📦 [FMP4_DIRECT] Segmento fMP4 diretto: {segment_url[-50:]}", "INFO", "AppCore")
                    enhanced_log(f"🔗 [FMP4_DIRECT] URL proxy: {proxy_url[:100]}...", "DEBUG", "AppCore")
                else:
                    ...
                    if headers_query:
                        proxy_url = f"http://127.0.0.1:7860/proxy/ts?url={quote(segment_url)}&stream_id={stream_id}&{headers_query}"
                    else:
                        proxy_url = f"http://127.0.0.1:7860/proxy/ts?url={quote(segment_url)}&stream_id={stream_id}"
                    
                    # Log specifico per segmenti DLHD (inclusi quelli codificati)
                    if is_dlhd_domain:
                        enhanced_log(f"🎭 [DLHD_SEGMENT] Segmento DLHD processato come TS: {segment_url.split('/')[-1][:30]}...", "INFO", "AppCore")
                        # Forza content-type corretto per segmenti DLHD
                    if dlhd_masked:
                        proxy_url += "&dlhd_masked=1"
                
                enhanced_log(f"✅ [SEGMENT_PROXY] Segmento trasformato in proxy: {segment_url[-50:]}", "DEBUG", "AppCore")
        ...
        segment_lines = [line for line in modified_m3u8 if line and not line.startswith('#') and line.strip()]
        if len(segment_lines) == 0:
            enhanced_log("⚠️ [M3U8_EMPTY] M3U8 vuoto - PROVO URL .m3u8 ALTERNATIVO", "WARNING", "AppCore")
            
            # Prova URL .m3u8 invece di .css
            if final_url.endswith('.css'):
                alt_url = final_url.replace('.css', '.m3u8')
                enhanced_log(f"🔄 [M3U8_ALT] Tentativo URL alternativo: {alt_url}", "INFO", "AppCore")
                ...
                try:
                    ...
                    if alt_response.status_code == 200:
                        ...
                        if alt_content.strip().startswith('#EXTM3U') and 'EXTINF' in alt_content:
                            ...
                            video_segments = [seg for seg in alt_segments 
                                            if not any(ext in seg.lower() for ext in ['.js', '.html', '.txt', '.css', '.json'])]
                            
                            if len(video_segments) > 0:
                                enhanced_log(f"✅ [M3U8_ALT] URL alternativo con {len(video_segments)} segmenti video", "INFO", "AppCore")
                                ...
                                for line in m3u_content.splitlines():
                                    ...
                                    if line.startswith("#EXT-X-KEY") and 'AES-128' in line:
                                        ...
                                        if is_subtitle_resource(segment_url):
                                            enhanced_log(f"[SUBTITLE_SKIP] Segmento sottotitoli scartato in M3U8 alternativo: {line}", "INFO", "AppCore")
                                            continue
                                ...
                                enhanced_log(f"✅ [M3U8_ALT] M3U8 alternativo processato con successo", "INFO", "AppCore")
                            else:
                                enhanced_log(f"⚠️ [M3U8_ALT] URL alternativo senza segmenti video validi", "WARNING", "AppCore")
                        else:
                            enhanced_log(f"⚠️ [M3U8_ALT] URL alternativo non è M3U8 valido", "WARNING", "AppCore")
                    else:
                        enhanced_log(f"⚠️ [M3U8_ALT] URL alternativo HTTP {alt_response.status_code}", "WARNING", "AppCore")
                        
                except Exception as alt_error:
                    enhanced_log(f"❌ [M3U8_ALT] Errore URL alternativo: {alt_error}", "ERROR", "AppCore")
        ...
        if dlhd_masked:
            ...
            final_m3u8 = '\n'.join(m3u8_lines) + "\n"
            enhanced_log(f"🎭 [DLHD_MASKED] Rimossa riga #EXT-X-KEY dall'M3U8 per dlhd_masked=1", "INFO", "AppCore")
        else:
            final_m3u8 = "\n".join(modified_m3u8) + "\n"
        
        enhanced_log(f"📄 [M3U8_FINAL] M3U8 finale ({len(final_m3u8)} caratteri):", "DEBUG", "AppCore")
        enhanced_log(f"📄 [M3U8_FINAL] Prime 500 caratteri: {final_m3u8[:500]}", "DEBUG", "AppCore")
        
        # ✅ VALIDAZIONE FINALE: Verifica che M3U8 abbia segmenti validi
        segment_count = len([line for line in final_m3u8.splitlines() if line and not line.startswith('#') and line.strip()])
        if segment_count == 0:
            enhanced_log("⚠️ [M3U8_VALIDATION] M3U8 finale senza segmenti - possibile problema", "WARNING", "AppCore")
        else:
            enhanced_log(f"✅ [M3U8_VALIDATION] M3U8 finale con {segment_count} segmenti validi", "INFO", "AppCore")
        ...
        if is_vavoo_request:
            ...
            enhanced_log(f"[VAVOO_M3U8_CACHE] Playlist salvata per refresh ({segment_count} segmenti)", "DEBUG", "AppCore")
...
def proxy_resolve(request=None, **kwargs):
    ...
    try:
        ...
        enhanced_log(f"Errore durante la risoluzione dell'URL: {str(e)}", "ERROR", "AppCore")
        return {
...
def proxy_init_fmp4(request=None, **kwargs):
    ...
    enhanced_log("🎬 [INIT_FMP4] Inizio processamento init fMP4", "INFO", "proxy_fmp4")
    ...
    try:
        ...
        enhanced_log(f"✅ [INIT_FMP4] Init scaricato: {len(response.content)} bytes", "INFO", "proxy_fmp4")
        ...
        enhanced_log(f"⚠️ [INIT_FMP4] Server init fallito: {e}", "WARNING", "proxy_fmp4")
        enhanced_log("🔧 [INIT_FMP4] Genero init sintetico per fMP4", "INFO", "proxy_fmp4")
        ...
        enhanced_log(f"✅ [INIT_FMP4] Init sintetico generato: {len(synthetic_init)} bytes", "INFO", "proxy_fmp4")
...
def proxy_ts(request=None, **kwargs):
    ...
    enhanced_log("🎬 [PROXY_TS] === INIZIO PROCESSAMENTO SEGMENTO TS ===", "INFO", "proxy_ts")
    
    # ✅ LOGGING DETTAGLIATO PER DEBUG
    enhanced_log(f"🔍 [PROXY_TS] Kwargs ricevuti: {list(kwargs.keys())}", "DEBUG", "proxy_ts")
    ...
    enhanced_log(f"📋 [PROXY_TS] URL estratto: {ts_url[-50:] if ts_url else 'VUOTO'}", "INFO", "proxy_ts")
    enhanced_log(f"📋 [PROXY_TS] Stream ID: {stream_id}", "INFO", "proxy_ts")
    enhanced_log(f"📋 [PROXY_TS] fMP4: {is_fmp4}, DLHD masked: {is_dlhd_masked}", "INFO", "proxy_ts")
    
    if not ts_url:
        enhanced_log("❌ [PROXY_TS] URL mancante - ERRORE CRITICO", "ERROR", "proxy_ts")
        return {
            'content': b'',
            'status': 400,
            'content_type': 'text/plain'
    ...
    enhanced_log(f"✅ [PROXY_TS] URL valido ricevuto, continuo processamento", "INFO", "proxy_ts")
    
    # ✅ RILEVAMENTO DLHD MIGLIORATO: Tutti i segmenti da domini DLHD sono potenzialmente video
    if not is_dlhd_masked:
        is_dlhd_masked = is_daddy_domain(ts_url) or 'playerfuncc.fun' in ts_url.lower()
        enhanced_log(f"🔍 [PROXY_TS] DLHD rilevato automaticamente: {is_dlhd_masked}", "DEBUG", "proxy_ts")
    
    enhanced_log(f"📦 [PROXY_TS] Richiesta segmento: {ts_url.split('/')[-1] if '/' in ts_url else ts_url[-30:]}", "INFO", "proxy_ts")
    if stream_id:
        enhanced_log(f"🆔 [PROXY_TS] Stream ID: {stream_id}", "INFO", "proxy_ts")
    if is_fmp4:
        enhanced_log(f"📹 [PROXY_TS] Modalità fMP4 attivata", "INFO", "proxy_ts")
    if is_dlhd_masked:
        enhanced_log(f"🎭 [DLHD_MASKED] Segmento DLHD rilevato (video mascherato)", "INFO", "proxy_ts")
    
    # Estrai headers custom
    enhanced_log(f"📝 [PROXY_TS] Estrazione headers custom...", "DEBUG", "proxy_ts")
    headers = {
        unquote(key[2:]).replace("_", "-"): unquote(value).strip()
        for key, value in kwargs.items()
        if key.lower().startswith("h_")
    }
    enhanced_log(f"📝 [PROXY_TS] Headers estratti: {len(headers)} elementi", "DEBUG", "proxy_ts")
    ...
    enhanced_log(f"⏰ [PROXY_TS] Timeout impostato: {timeout}s", "DEBUG", "proxy_ts")
    
    try:
        enhanced_log(f"🔄 [PROXY_TS] === INIZIO DOWNLOAD SEGMENTO ===", "INFO", "proxy_ts")
        
        # ✅ CORREZIONE CRITICA: Usa sessione persistente DLHD se disponibile
        dlhd_session = get_dlhd_session()
        if dlhd_session and any(domain in ts_url.lower() for domain in ['kiko2.ru', 'giokko.ru']):
            enhanced_log(f"🍪 [PROXY_TS] Usando sessione persistente DLHD", "INFO", "proxy_ts")
            response = dlhd_session.get(ts_url, headers=headers, timeout=timeout, verify=False)
        else:
            enhanced_log(f"🌐 [PROXY_TS] Usando richiesta standard", "DEBUG", "proxy_ts")
            response = make_persistent_request(ts_url, headers=headers, timeout=timeout)
        
        enhanced_log(f"✅ [PROXY_TS] Risposta HTTP: {response.status_code}", "INFO", "proxy_ts")
        response.raise_for_status()
        ts_content = response.content
        
        enhanced_log(f"✅ [PROXY_TS] === SEGMENTO SCARICATO: {len(ts_content)} bytes ===", "INFO", "proxy_ts")
        ...
        if non_ts_content_type and not is_dlhd_masked:
            enhanced_log(f"[PROXY_TS] Risorsa non-TS servita senza decrittazione: {non_ts_content_type}", "INFO", "proxy_ts")
            return {
                'content': ts_content,
                'status': 200,
        ...
        if is_direct_media_url(ts_url):
            enhanced_log(f"🎬 [PROXY_TS] Media diretto servito senza conversione TS: {ts_url[-60:]}", "INFO", "proxy_ts")
            return {
                'content': ts_content,
                'status': 200,
                'content_type': 'video/mp4'
        ...
        if is_dlhd_masked:
            enhanced_log(f"🎭 [DLHD_MASKED] === PROCESSAMENTO SEGMENTO MASCHERATO ===", "INFO", "proxy_ts")
            # Verifica che sia effettivamente un segmento TS (sync byte 0x47)
            if len(ts_content) > 0:
                enhanced_log(f"🔍 [DLHD_MASKED] Primo byte: 0x{ts_content[0]:02x}", "DEBUG", "proxy_ts")
                if not is_valid_ts_payload(ts_content):
                    enhanced_log(f"⚠️ [DLHD_MASKED] Segmento non ha sync byte TS, potrebbe essere criptato", "WARNING", "proxy_ts")
                    # Prova decrittazione se disponibile
                    if stream_id and AES_AVAILABLE:
                        enhanced_log(f"🔐 [DLHD_MASKED] Tentativo decrittazione segmento mascherato", "INFO", "proxy_ts")
                        aes_key = get_aes_key_for_stream(stream_id, headers, ts_url)
                        if aes_key:
                            enhanced_log(f"🔑 [DLHD_MASKED] Chiave AES trovata, decrittazione in corso", "INFO", "proxy_ts")
                            decrypted = decrypt_ts_if_needed(ts_content, stream_id, headers, ts_url)
                            if decrypted != ts_content and is_valid_ts_payload(decrypted):
                                ts_content = decrypted
                                enhanced_log(f"✅ [DLHD_MASKED] Segmento mascherato decriptato con successo", "INFO", "proxy_ts")
                            else:
                                enhanced_log(f"⚠️ [DLHD_MASKED] Decrittazione fallita o non necessaria", "WARNING", "proxy_ts")
                        else:
                            enhanced_log(f"❌ [DLHD_MASKED] Chiave AES non disponibile", "ERROR", "proxy_ts")
                else:
                    enhanced_log(f"✅ [DLHD_MASKED] Segmento mascherato già in formato TS valido", "INFO", "proxy_ts")
            else:
                enhanced_log(f"❌ [DLHD_MASKED] Segmento vuoto!", "ERROR", "proxy_ts")
        
        # ✅ GESTIONE fMP4: Per Freeshot e altri provider fMP4
        elif is_fmp4 or '.fmp4' in ts_url.lower():
            enhanced_log(f"📹 [FMP4_PROCESS] Processamento segmento fMP4", "DEBUG", "proxy_ts")
            
            # Per Enigma2, converti fMP4 in TS se necessario
            if stream_id and stream_id in STREAM_KEY_INFO:
                stream_info = STREAM_KEY_INFO[stream_id]
                if stream_info.get('is_freeshot', False):
                    # Per Freeshot, mantieni fMP4 nativo (Enigma2 moderno lo supporta)
                    enhanced_log(f"✅ [FMP4_NATIVE] Freeshot: mantengo fMP4 nativo per Enigma2", "INFO", "proxy_ts")
                    return {
                        'content': ts_content,
                        'status': 200,
                        'content_type': 'video/mp4'
                    ...
                    enhanced_log(f"🔄 [FMP4_CONVERT] Conversione fMP4 → TS per compatibilità", "INFO", "proxy_ts")
        ...
        enhanced_log(f"🔍 [PROXY_TS] === VERIFICA FORMATO SEGMENTO ===", "INFO", "proxy_ts")
        if len(ts_content) > 0:
            enhanced_log(f"🔍 [PROXY_TS] Primo byte segmento: 0x{ts_content[0]:02x}", "DEBUG", "proxy_ts")
            if is_valid_ts_payload(ts_content):
                enhanced_log(f"✅ [PROXY_TS] Segmento già decriptato (sync byte 0x47)", "INFO", "proxy_ts")
                return {
                    'content': ts_content,
                    'status': 200,
                ...
                enhanced_log(f"⚠️ [PROXY_TS] Segmento richiede decrittazione (primo byte: 0x{ts_content[0]:02x})", "WARNING", "proxy_ts")
        else:
            enhanced_log(f"❌ [PROXY_TS] Segmento vuoto!", "ERROR", "proxy_ts")
        
        # Prova decrittazione AES se necessario
        enhanced_log(f"🔐 [PROXY_TS] === INIZIO DECRITTAZIONE AES ===", "INFO", "proxy_ts")
        if stream_id and AES_AVAILABLE:
            enhanced_log(f"🔑 [PROXY_TS] Ricerca chiave AES per stream {stream_id}", "INFO", "proxy_ts")
            
            # ✅ CORREZIONE CRITICA: Chiama sempre decrypt_ts_if_needed
            decrypted = decrypt_ts_if_needed(ts_content, stream_id, headers, ts_url)
            if decrypted != ts_content:
                ts_content = decrypted
                enhanced_log(f"✅ [PROXY_TS] DECRITTAZIONE RIUSCITA: {len(ts_content)} bytes", "INFO", "proxy_ts")
                if len(ts_content) > 0:
                    enhanced_log(f"✅ [PROXY_TS] Sync byte dopo decrittazione: 0x{ts_content[0]:02x}", "INFO", "proxy_ts")
            else:
                enhanced_log(f"⚠️ [PROXY_TS] Decrittazione non ha modificato il contenuto", "WARNING", "proxy_ts")
        else:
            if not stream_id:
                enhanced_log(f"⚠️ [PROXY_TS] Stream ID mancante - decrittazione saltata", "WARNING", "proxy_ts")
            if not AES_AVAILABLE:
                enhanced_log(f"⚠️ [PROXY_TS] AES non disponibile - decrittazione saltata", "WARNING", "proxy_ts")
        
        # Determina content-type finale
        enhanced_log(f"📝 [PROXY_TS] === PREPARAZIONE RISPOSTA FINALE ===", "INFO", "proxy_ts")
        if is_fmp4 or '.fmp4' in ts_url.lower() or is_direct_media_url(ts_url):
            content_type = 'video/mp4'
            enhanced_log(f"📹 [PROXY_TS] Content-Type: video/mp4 (fMP4)", "INFO", "proxy_ts")
        else:
            # Sia per TS normali che per .html DLHD, usa video/mp2t
            content_type = 'video/mp2t'
            enhanced_log(f"📺 [PROXY_TS] Content-Type: video/mp2t (TS)", "INFO", "proxy_ts")
        
        # Verifica finale del contenuto
        if len(ts_content) > 0:
            enhanced_log(f"✅ [PROXY_TS] === SEGMENTO PRONTO: {len(ts_content)} bytes, sync: 0x{ts_content[0]:02x} ===", "INFO", "proxy_ts")
        else:
            enhanced_log(f"❌ [PROXY_TS] === SEGMENTO VUOTO - PROBLEMA CRITICO ===", "ERROR", "proxy_ts")
        
        if is_dlhd_masked:
            enhanced_log(f"✅ [DLHD_MASKED] Segmento DLHD pronto come TS: {len(ts_content)} bytes", "INFO", "proxy_ts")
        ...
        enhanced_log(f"❌ [PROXY_TS] === ERRORE CRITICO DURANTE PROCESSAMENTO ===", "ERROR", "proxy_ts")
        enhanced_log(f"❌ [PROXY_TS] Errore: {type(e).__name__}: {str(e)}", "ERROR", "proxy_ts")
        
        # Log stack trace per debug
        import traceback
        enhanced_log(f"🔍 [PROXY_TS] Stack trace: {traceback.format_exc()}", "ERROR", "proxy_ts")
        
        # ✅ FALLBACK MIGLIORATO: Segmento TS vuoto ma valido per evitare interruzioni
        # Per DLHD, genera un pacchetto TS con PAT corretto
        if is_dlhd_masked:
            enhanced_log(f"🎭 [DLHD_FALLBACK] Generazione fallback TS per DLHD", "WARNING", "proxy_ts")
            ...
            enhanced_log(f"🎭 [DLHD_FALLBACK] Fallback TS DLHD generato: {len(fallback_ts)} bytes", "WARNING", "proxy_ts")
        else:
            enhanced_log(f"🆘 [PROXY_TS] Generazione fallback TS standard", "WARNING", "proxy_ts")
            fallback_ts = b'\x47\x1F\xFF\x10' + b'\xFF' * 184
            enhanced_log(f"🆘 [PROXY_TS] Fallback TS generato: {len(fallback_ts)} bytes", "WARNING", "proxy_ts")
...
def cleanup_expired_segments():
    ...
    for stream_id in list(STREAM_KEY_INFO.keys()):
        ...
        if 'init_segment' in stream_info and 'init_timestamp' in stream_info:
            if current_time - stream_info['init_timestamp'] > 120:  # 2 minuti per stream live
    ...
    if expired_keys or old_init_count > 0:
        enhanced_log(f"🧹 [SEGMENT_CLEANUP] Rimossi {len(expired_keys)} segmenti scaduti (3s) e {old_init_count} init segments vecchi dalla cache", "DEBUG", "AppCore")
...
def clear_stream_cache():
    ...
    enhanced_log(f"🧹 [CACHE_CLEAR] Pulizia completa: {old_stream_count} stream, {old_key_count} chiavi AES", "INFO", "AppCore")
...
def is_valid_video_segment(url, strict_mode=False, is_dlhd=False):
    ...
    valid_extensions = ('.ts', '.m3u8', '.mp4', '.mkv', '.webm', '.flv', '.mov', '.avi')
    # Estensioni non-video da escludere (ma .html/.css è permesso per DLHD)
    invalid_extensions = ('.php', '.js', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.txt', '.xml', '.json')
    
    # Per DLHD, .html/.css/.js/.txt è considerato valido (segmenti video mascherati)
    if is_dlhd and any(ext in url_lower for ext in ['.html', '.css', '.js', '.txt']):
        enhanced_log(f"✅ [DLHD_MASKED] Segmento DLHD accettato (video mascherato): {url[-50:]}", "INFO", "AppCore")
        return True
    
    # Controlla estensioni non-video - SCARTA SEMPRE (eccetto .html/.css per DLHD)
    for ext in invalid_extensions:
        if url_lower.endswith(ext):
            enhanced_log(f"⚠️ [SEGMENT_VALIDATION] Segmento non-video scartato: {url[-50:]}", "WARNING", "AppCore")
            return False
    
    # Se non è DLHD, scarta anche .html/.css/.js/.txt
    if not is_dlhd and any(ext in url_lower for ext in ['.html', '.css', '.js', '.txt']):
        enhanced_log(f"⚠️ [SEGMENT_VALIDATION] Segmento .html/.css/.js/.txt scartato (non-DLHD): {url[-50:]}", "WARNING", "AppCore")
        return False
    ...
    # Se non ha estensione ma contiene 'playlist' o 'm3u', accetta (potrebbe essere M3U8 senza estensione)
    ...
    if strict_mode:
        ...
        enhanced_log(f"⚠️ [SEGMENT_VALIDATION] Segmento senza estensione valida scartato: {url[-50:]}", "WARNING", "AppCore")
        return False
    else:
        # In modalità permissiva, accetta (potrebbe essere TS senza estensione)
        enhanced_log(f"ℹ️ [SEGMENT_VALIDATION] Segmento senza estensione accettato (modalità permissiva): {url[-50:]}", "INFO", "AppCore")
        return True


def extract_key_info_from_m3u8(m3u8_content, stream_id, base_url, stream_headers=None, resolved_data=None):
    ...
    if is_daddy:
        enhanced_log("🔍 [FASE_2] Analisi della playlist per chiave, IV e segmento", "INFO", "AppCore")
    elif is_freeshot:
        enhanced_log("🔍 [FREESHOT_M3U8] Analisi playlist fMP4 (non criptata)", "INFO", "AppCore")
    
    # Chiave AES non più scaricata dall'extractor, sarà scaricata da AppCore quando serve
    ...
    try:
        ...
        for line in m3u8_content:
            if line.startswith('#EXT-X-MEDIA-SEQUENCE'):
                try:
                    ...
                    enhanced_log(f"🔢 [SEQUENCE] Sequenza iniziale: {segment_sequence}", "DEBUG", "AppCore")
                except Exception as sequence_error:
                    enhanced_log(f"[SEQUENCE] Sequenza media non valida: {sequence_error}", "DEBUG", "AppCore")
        ...
        for line in m3u8_content:
            if line.startswith('#EXT-X-KEY') and 'AES-128' in line:
                ...
                if not uri_match:
                    enhanced_log(f"⚠️ [KEY_EXTRACTION] URI non trovato nella linea KEY: {line[:100]}", "WARNING", "AppCore")
                    continue
                ...
                # L'URL viene salvato così com'è - se diventa obsoleto, verrà gestito in get_aes_key_for_stream
                if is_daddy and 'giokko.ru' in key_uri:
                    enhanced_log(f"🔑 [KEY_INFO] URL chiave DaddyLive dinamico: {key_uri[:80]}...", "DEBUG", "AppCore")
                iv_bytes = None
                if iv_match:
                    iv_value = iv_match.group(1).strip('"\'')
                    enhanced_log(f"🔢 [IV] Valore IV estratto: {iv_value}", "DEBUG", "AppCore")
                    if iv_value.startswith('0x'):
                        iv_value = iv_value[2:]
                    if all(c in '0123456789abcdefABCDEF' for c in iv_value):
                        try:
                            iv_bytes = bytes.fromhex(iv_value)
                            enhanced_log(f"🔢 [IV] Convertito da hex: {iv_bytes.hex()}", "DEBUG", "AppCore")
                        except Exception as e:
                            iv_bytes = iv_value.encode('latin-1')
                            enhanced_log(f"⚠️ [IV] Fallita conversione hex, usato come stringa: {iv_bytes.hex()}", "WARNING", "AppCore")
                    else:
                        iv_bytes = iv_value.encode('latin-1')
                        enhanced_log(f"🔢 [IV] Interpretato come stringa ASCII: {iv_bytes.hex()}", "DEBUG", "AppCore")
                    if len(iv_bytes) != 16:
                        if len(iv_bytes) < 16:
                            iv_bytes = iv_bytes + bytes(16 - len(iv_bytes))
                            enhanced_log(f"⚠️ [IV] IV troppo corto, allungato a: {iv_bytes.hex()}", "WARNING", "AppCore")
                        else:
                            iv_bytes = iv_bytes[:16]
                            enhanced_log(f"⚠️ [IV] IV troppo lungo, troncato a: {iv_bytes.hex()}", "WARNING", "AppCore")
                if not iv_bytes:
                    iv_bytes = hashlib.md5(stream_id.encode()).digest()[:16]
                    enhanced_log(f"⚠️ [KEY_EXTRACTION] IV non valido, usando IV generato: {iv_bytes.hex()}", "WARNING", "AppCore")
                if not isinstance(iv_bytes, bytes):
                    iv_bytes = str(iv_bytes).encode('latin-1')
                if len(iv_bytes) < 16:
                    iv_bytes = iv_bytes + bytes(16 - len(iv_bytes))
                    enhanced_log(f"⚠️ [KEY_EXTRACTION] IV troppo corto, allungato con zeri: {iv_bytes.hex()}", "WARNING", "AppCore")
                elif len(iv_bytes) > 16:
                    iv_bytes = iv_bytes[:16]
                    enhanced_log(f"⚠️ [KEY_EXTRACTION] IV troppo lungo, troncato a: {iv_bytes.hex()}", "WARNING", "AppCore")
                iv_hex = iv_bytes.hex()
                ...
                if is_daddy:
                    enhanced_log(f"✅ [FASE_2] Chiave trovata: {key_uri[:80]}...", "INFO", "AppCore")
                    enhanced_log(f"✅ [FASE_2] IV base trovato: {iv_hex}", "INFO", "AppCore")
                    enhanced_log(f"✅ [FASE_2] Sequenza iniziale: {segment_sequence}", "INFO", "AppCore")
                    if first_segment:
                        full_segment_url = urljoin(base_url, first_segment)
                        enhanced_log(f"🎯 [FASE_2] Primo segmento trovato: {first_segment[:80]}...", "DEBUG", "AppCore")
                        enhanced_log(f"🔗 [FASE_2] URI Chiave assoluto: {key_uri}", "DEBUG", "AppCore")
                        enhanced_log(f"🔗 [FASE_2] URL Segmento assoluto: {full_segment_url}", "DEBUG", "AppCore")
                else:
                    enhanced_log(f"🔑 Info chiave salvata per stream {stream_id}", "INFO", "AppCore")
                    enhanced_log(f"🔑 [KEY_EXTRACTION] URI: {key_uri[:80]}...", "DEBUG", "AppCore")
                    enhanced_log(f"🔑 [KEY_EXTRACTION] IV: {iv_hex}", "DEBUG", "AppCore")
                return True
        if is_freeshot:
            enhanced_log(f"ℹ️ [FREESHOT_M3U8] Nessuna chiave AES necessaria per fMP4 (stream non criptato)", "INFO", "AppCore")
        else:
            enhanced_log(f"⚠️ [KEY_EXTRACTION] Nessuna chiave AES-128 valida trovata nel M3U8", "WARNING", "AppCore")
        return False
    except Exception as e:
        enhanced_log(f"❌ Errore estrazione info chiave: {e}", "ERROR", "AppCore")
        return False
...
def get_aes_key_for_stream(stream_id, headers, segment_url=None):
    ...
    enhanced_log(f"🔑 [AES_KEY] === INIZIO RICERCA CHIAVE AES per stream {stream_id} ===", "INFO", "AppCore")
    
    if stream_id not in STREAM_KEY_INFO:
        enhanced_log(f"❌ [AES_KEY] Stream {stream_id} non trovato in STREAM_KEY_INFO", "ERROR", "AppCore")
        return None
    
    key_info = STREAM_KEY_INFO[stream_id]
    key_uri = key_info.get('key_uri')
    if not key_uri:
        enhanced_log(f"❌ [AES_KEY] key_uri mancante per stream {stream_id}", "ERROR", "AppCore")
        return None
    
    enhanced_log(f"🔍 [AES_KEY] key_uri trovato: {key_uri[-30:]}", "DEBUG", "AppCore")
    
    cache_key = f"{stream_id}_{key_uri}"
    stream_headers = key_info.get('headers', {})
    enhanced_log(f"📝 [AES_KEY] Headers stream: {len(stream_headers)} elementi", "DEBUG", "AppCore")
    
    # RILEVAMENTO CAMBIO CHIAVE: Se l'URL della chiave è cambiato, invalida cache
    ...
    if last_key_uri and current_key_uri != last_key_uri:
        enhanced_log(f"🔄 [KEY_CHANGE] Rilevato cambio chiave: {last_key_uri[-10:]} → {current_key_uri[-10:]}", "INFO", "AppCore")
    ...
    if cache_key in AES_KEY_CACHE:
        enhanced_log(f"✅ [AES_KEY] Chiave trovata in cache: {cache_key}", "INFO", "AppCore")
        ...
        if current_time - last_heartbeat > 30:
            ...
            if heartbeat_url:
                enhanced_log(f"💓 [HEARTBEAT] Invio heartbeat: {heartbeat_url[-30:]}", "DEBUG", "AppCore")
                try:
                    hb_headers = {
                    ...
                    dlhd_session = get_dlhd_session()
                    if dlhd_session:
                        enhanced_log(f"🍪 [HEARTBEAT] Usando sessione DLHD persistente", "DEBUG", "AppCore")
                        response = dlhd_session.get(heartbeat_url, headers=hb_headers, timeout=2, verify=False)
                    else:
                        response = make_persistent_request(heartbeat_url, headers=hb_headers, timeout=2)
                    
                    if response.status_code == 200:
                        key_info['last_heartbeat'] = current_time
                        enhanced_log(f"💓 [HEARTBEAT] Heartbeat riuscito per stream {stream_id}", "DEBUG", "AppCore")
                    else:
                        enhanced_log(f"⚠️ [HEARTBEAT] Heartbeat fallito: {response.status_code}", "WARNING", "AppCore")
                except Exception as e:
                    enhanced_log(f"⚠️ [HEARTBEAT] Errore heartbeat: {e}", "WARNING", "AppCore")
        
        return AES_KEY_CACHE[cache_key]
    
    # Scarica nuova chiave AES
    enhanced_log(f"🔄 [AES_KEY] === DOWNLOAD NUOVA CHIAVE AES ===", "INFO", "AppCore")
    try:
        enhanced_log(f"🔑 [NEW_KEY] Scarico nuova chiave: {key_uri[-20:]}", "INFO", "AppCore")
        key_headers = {k: v for k, v in stream_headers.items() if k != 'Heartbeat-Url'}
        enhanced_log(f"📝 [NEW_KEY] Headers per download: {list(key_headers.keys())}", "DEBUG", "AppCore")
        ...
        if dlhd_session and 'kiko2.ru' in key_uri:
            enhanced_log(f"🍪 [DLHD_KEY] Usando sessione persistente DLHD per chiave AES", "INFO", "AppCore")
            response = dlhd_session.get(key_uri, headers=key_headers, timeout=timeout, verify=False)
        else:
            enhanced_log(f"🌐 [NEW_KEY] Usando richiesta standard", "DEBUG", "AppCore")
            response = make_persistent_request(key_uri, headers=key_headers, timeout=timeout)
        
        enhanced_log(f"📊 [NEW_KEY] Risposta HTTP: {response.status_code}", "INFO", "AppCore")
        
        if response.status_code != 200:
            enhanced_log(f"❌ [NEW_KEY] Chiave AES fallita (HTTP {response.status_code})", "ERROR", "AppCore")
            
            # ✅ NUOVO: Se fallisce, prova a invalidare cache DLHD per forzare refresh
            if DLHD_AVAILABLE and dlhd_extractor and response.status_code in [403, 404]:
                try:
                    if hasattr(dlhd_extractor, 'invalidate_cache_for_url') and segment_url:
                        dlhd_extractor.invalidate_cache_for_url(segment_url)
                        enhanced_log(f"🗑️ [DLHD_INVALIDATE] Cache DLHD invalidata per errore chiave", "INFO", "AppCore")
                except Exception:
                    pass
        ...
        enhanced_log(f"🔑 [NEW_KEY] Chiave scaricata: {len(aes_key)} bytes", "INFO", "AppCore")
        
        if len(aes_key) == 16:
            AES_KEY_CACHE[cache_key] = aes_key
            key_info['last_heartbeat'] = time.time()
            key_info['last_key_uri'] = key_uri
            enhanced_log(f"✅ [NEW_KEY] === NUOVA CHIAVE AES SALVATA ({len(aes_key)} bytes) ===", "INFO", "AppCore")
            return aes_key
        else:
            enhanced_log(f"❌ [NEW_KEY] Chiave AES dimensione invalida: {len(aes_key)} bytes (attesi 16)", "ERROR", "AppCore")
        
        return None
        
    except Exception as e:
        enhanced_log(f"❌ [NEW_KEY] === ERRORE DOWNLOAD CHIAVE AES: {type(e).__name__}: {str(e)} ===", "ERROR", "AppCore")
        import traceback
        enhanced_log(f"🔍 [NEW_KEY] Stack trace: {traceback.format_exc()}", "ERROR", "AppCore")
        return None


# Sostituisci la funzione decrypt_ts_segment (riga ~1062) con questa versione corretta:
def decrypt_ts_segment(ts_content, aes_key, stream_id):
    ...
    try:
        ...
        if stream_id in STREAM_KEY_INFO and 'iv' in STREAM_KEY_INFO[stream_id]:
            ...
            if isinstance(iv, str):
                try:
                    ...
                    if all(c in '0123456789abcdefABCDEF' for c in iv_str):
                        iv = bytes.fromhex(iv_str)
                        enhanced_log(f"🔢 [IV] Convertito da hex: {iv.hex()}", "DEBUG", "proxy_ts")
                    else:
                        # Se non è esadecimale valido, usa come stringa ASCII
                        iv = iv_str.encode('latin-1')
                        enhanced_log(f"🔢 [IV] Interpretato come stringa: {iv.hex()}", "DEBUG", "proxy_ts")
                except Exception as e:
                    enhanced_log(f"⚠️ [IV] Errore conversione IV: {e}", "WARNING", "proxy_ts")
                    iv = None
            ...
            if iv is not None:
                if len(iv) < 16:
                    ...
                    enhanced_log(f"⚠️ [IV] IV troppo corto, allungato: {iv.hex()}", "WARNING", "proxy_ts")
                elif len(iv) > 16:
                    # Se è troppo lungo, prendi i primi 16 byte
                    iv = iv[:16]
                    enhanced_log(f"⚠️ [IV] IV troppo lungo, troncato: {iv.hex()}", "WARNING", "proxy_ts")
        ...
        enhanced_log(f"🔢 [IV] IV finale per decifratura: {iv.hex()}", "DEBUG", "proxy_ts")
    
        # Log IV e chiave per debug
        is_daddy = STREAM_KEY_INFO.get(stream_id, {}).get('is_daddy', False)
        if is_daddy:
            enhanced_log(f"🔐 [DECRYPT] IV (hex): {iv.hex()}", "DEBUG", "AppCore")
            enhanced_log(f"🔐 [DECRYPT] Chiave (hex): {aes_key.hex()}", "DEBUG", "AppCore")
        ...
        try:
            ...
            # Se il sync byte non è corretto, prova a decifrare senza rimuovere il padding
            if AES_MODULE == "cryptography":
                cipher = Cipher(algorithms.AES(aes_key), modes.CBC(iv), backend=default_backend())
                decryptor = cipher.decryptor()
                decrypted = decryptor.update(ts_content) + decryptor.finalize()
            ...
            enhanced_log(f"❌ Errore durante la decrittazione: {decrypt_error}", "ERROR", "proxy_ts")
            return ts_content
    except Exception as e:
...
def get_persistent_session(proxy_url=None):
    ...
    with SESSION_LOCK:
        ...
        if session is not None:
            session_age = current_time - getattr(session, "_sp_created_at", current_time)
            ...
            if session_age > SESSION_MAX_AGE or requests_count >= SESSION_MAX_REQUESTS:
                try:
                    ...
                    enhanced_log(f"Errore chiusura sessione scaduta {pool_key}: {close_error}", "DEBUG", "AppCore")
        ...
        if session is None:
            ...
            if session is None:
                enhanced_log(f"Impossibile creare sessione per: {pool_key}", "get_persistent_session", "AppCore")
                return None
            ...
            enhanced_log(f"Nuova sessione persistente creata per: {pool_key}", "get_persistent_session", "AppCore")

        SESSION_POOL[pool_key]._sp_requests_count = getattr(SESSION_POOL[pool_key], "_sp_requests_count", 0) + 1
...
def make_persistent_request(url, headers=None, timeout=None, proxy_url=None, **kwargs):
    ...
    enhanced_log(f"🌐 [PERSISTENT_REQUEST] URL: {url[:100]}...", "DEBUG", "AppCore")
    
    # ✅ CORREZIONE CRITICA: Usa sessione DLHD persistente per domini kiko2.ru
    dlhd_session = get_dlhd_session()
    if dlhd_session and any(domain in url.lower() for domain in ['kiko2.ru', 'giokko.ru']):
        enhanced_log(f"🍪 [DLHD_SESSION] Usando sessione persistente DLHD", "DEBUG", "AppCore")
        try:
            response = dlhd_session.get(url, headers=headers, timeout=timeout or REQUEST_TIMEOUT, verify=False, **kwargs)
            enhanced_log(f"✅ [DLHD_SESSION] Risposta: {response.status_code}", "DEBUG", "AppCore")
            return response
        except Exception as e:
    ...
    for attempt in range(3):
        try:
            ...
            if session is None:
                enhanced_log("❌ Impossibile ottenere sessione persistente", "ERROR", "AppCore")
                raise Exception("Impossibile ottenere sessione persistente")
            
            enhanced_log(f"🔄 [PERSISTENT_REQUEST] Tentativo {attempt + 1}/3", "DEBUG", "AppCore")
            
            if request_headers:
                response = session.get(url, headers=request_headers, timeout=final_timeout, verify=VERIFY_SSL, **kwargs)
            else:
                response = session.get(url, timeout=final_timeout, verify=VERIFY_SSL, **kwargs)
            
            enhanced_log(f"✅ [PERSISTENT_REQUEST] Risposta: {response.status_code}", "DEBUG", "AppCore")
            ...
            enhanced_log(f"❌ [PERSISTENT_REQUEST] Errore connessione tentativo {attempt + 1}: {e}", "ERROR", "AppCore")
            with SESSION_LOCK:
                pool_key = proxy_url if proxy_url else 'default'
            ...
            enhanced_log(f"❌ [PERSISTENT_REQUEST] Errore generico: {e}", "ERROR", "AppCore")
            raise
...
def proxy_key(request=None, **kwargs):
    ...
    try:
        enhanced_log(f"🔑 [PROXY_KEY] Scarico chiave: {key_url[-30:]}", "INFO", "AppCore")
        
        # Usa sessione DLHD persistente se disponibile
        dlhd_session = get_dlhd_session()
        if dlhd_session and 'kiko2.ru' in key_url:
            enhanced_log(f"🍪 [PROXY_KEY] Usando sessione persistente DLHD", "DEBUG", "AppCore")
            response = dlhd_session.get(key_url, headers=headers, timeout=8, verify=False)
        else:
            response = make_persistent_request(key_url, headers=headers, timeout=8)
        
        response.raise_for_status()
        key_content = response.content
        
        if len(key_content) == 16:
            enhanced_log(f"✅ [PROXY_KEY] Chiave AES scaricata: {len(key_content)} bytes", "INFO", "AppCore")
            return {
                'content': key_content,
                'status': 200,
            ...
            enhanced_log(f"❌ [PROXY_KEY] Chiave dimensione invalida: {len(key_content)} bytes", "ERROR", "AppCore")
            return {
...
class ConfigManager:
        ...
        for path in config_paths:
            if os.path.exists(path):
                try:
                    ...
                    enhanced_log(f"⚠️ Errore nel caricamento di {path}: {e}", "load_config", "AppCore")

        if not loaded_path:
            enhanced_log("ℹ️ Nessun file di configurazione trovato. Uso i valori di default.", "load_config", "AppCore")

        proxy_str = config.get('PROXY', '')
        PROXY_LIST = [_normalize_proxy_url(p) for p in proxy_str.split(',') if p.strip()] if proxy_str else []
        if PROXY_LIST:
            enhanced_log(f"ℹ️ Proxy generali configurati: {len(PROXY_LIST)}", "load_config", "AppCore")

        daddy_proxy_str = config.get('DADDY_PROXY', '')
        DADDY_PROXY_LIST = [_normalize_proxy_url(p) for p in daddy_proxy_str.split(',') if p.strip()] if daddy_proxy_str else []
        if DADDY_PROXY_LIST:
            enhanced_log(f"ℹ️ Proxy DaddyLive configurati: {len(DADDY_PROXY_LIST)}", "load_config", "AppCore")
    ...
    def save_config(self, config):
        ...
        try:
            with open(path_to_save, 'w', encoding='utf-8') as f:
                config_to_save = {k: config.get(k) for k in self.default_config.keys() if k in config}
                json.dump(config_to_save, f, indent=4)
            enhanced_log(f"✅ Configurazione salvata in: {path_to_save}", "save_config", "AppCore")
            return True
        except IOError as e:
            enhanced_log(f"❌ Errore nel salvataggio della configurazione: {e}", "save_config", "AppCore")
            return False
    ...
    def apply_config_to_app(self, config):
        ...
        try:
            ...
            # Se Flask non è disponibile o non siamo in un contesto di app, ignora.
            return True
...
class AppCoreNoCache:
    ...
    def handle_request(self, route_name, *args, **kwargs):
        ...
        enhanced_log(f"🔍 [DEBUG] handle_request kwargs: {kwargs}", "DEBUG", "AppCore")
...
def service_monitor_callback(route_name, *args, **kwargs):
    enhanced_log(f"🔄 ServiceMonitor → AppCore: {route_name}", "INFO", "AppCore")
    enhanced_log(f"🔍 [DEBUG] Args ricevuti: args={args}, kwargs={kwargs}", "DEBUG", "AppCore")

    try:
        app_core = AppCoreNoCache()

        # CORREZIONE CRITICA: Debug e fix del passaggio parametri
        enhanced_log(f"🔍 [DEBUG] Prima di handle_request: kwargs={kwargs}", "DEBUG", "AppCore")
...
enhanced_log("🚀 AppCoreSC NoCache completamente inizializzato", "INFO", "AppCore")
