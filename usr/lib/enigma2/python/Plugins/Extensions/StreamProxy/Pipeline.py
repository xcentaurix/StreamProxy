...
class Pipeline:
    ...
    def process_and_write(self, content, content_type, source_url=None):
        ...
        try:
            enhanced_log(
                f"Pipeline processing: size={
                    len(content)} bytes, type={content_type}",
                "INFO",
                "PIPELINE")

            if content_type == "application/vnd.apple.mpegurl":
                # Verifica che il contenuto sia un M3U8 valido con segmenti
                # video
                if self._validate_m3u8_content(content):
                    ...
                    # Notifica ServiceMonitor con delay per evitare race
                    # condition
                    ...
                    enhanced_log(
                        "❌ Contenuto M3U8 non valido",
                        "ERROR",
                        "PIPELINE")
                    return False
            else:
                enhanced_log(
                    f"⚠️ Tipo contenuto non supportato: {content_type}",
                    "WARNING",
                    "PIPELINE")
                return False
    ...
    def _validate_m3u8_content(self, content):
        ...
        try:
            ...
            if not content.startswith('#EXTM3U'):
                enhanced_log(
                    "Contenuto non è un M3U8 valido",
                    "ERROR",
                    "PIPELINE")
                return False
            ...
            for line in lines:
                ...
                if line and not line.startswith('#'):
                    ...
                    if not is_non_video and (
                            '.ts' in line_lower or 'segment' in line_lower or '.m4s' in line_lower):
                        video_segments += 1
                        enhanced_log(
                            f"Segmento video valido trovato: {line[:100]}...", "DEBUG", "PIPELINE")
                    else:
                        enhanced_log(
                            f"Segmento non-video scartato: {line[:100]}...", "WARNING", "PIPELINE")

            enhanced_log(
                f"Trovati {video_segments} segmenti video validi",
                "INFO",
                "PIPELINE")
            ...
            enhanced_log(
                f"Errore nella validazione M3U8: {
                    str(e)}", "ERROR", "PIPELINE")
            return False
    ...
    def _notify_service_monitor(self):
        ...
        try:
            ...
            result = AppCore.service_monitor_callback(
                '/service/notify_m3u', path=self.output_path)
            enhanced_log(
                f"Notifica inviata a ServiceMonitor per il file: {
                    self.output_path}", "INFO", "PIPELINE")
            return result

        except Exception as e:
            enhanced_log(
                f"Errore nella notifica a ServiceMonitor: {
                    str(e)}", "ERROR", "PIPELINE")
            return False
