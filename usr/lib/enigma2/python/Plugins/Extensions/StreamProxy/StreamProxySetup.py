...
class StreamProxySetup(Screen, ConfigListScreen):
    skin = """
    <screen position="center,center" size="800,600" backgroundColor="#CC1a2a4a">
        <widget name="config" position="5,5" size="792,539" scrollbarMode="showOnDemand" backgroundColor="#CC1a2a4a" font="Regular; 50" itemHeight="45" />
        <ePixmap pixmap="skin_default/buttons/red.png" position="0,560" size="140,40" alphatest="on" />
        <ePixmap pixmap="skin_default/buttons/green.png" position="204,560" size="140,40" alphatest="on" />
        <widget name="key_red" position="40,560" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#9f1313" transparent="1" />
        <widget name="key_green" position="250,560" zPosition="1" size="140,40" font="Regular;20" halign="center" valign="center" backgroundColor="#1f771f" transparent="1" />
    </screen>"""
    ...
    def _saveProxyConfig(self):
        cfg = config.plugins.streamproxy
        data = {
            "attivaProxyEsterno": "YES" if cfg.attivaProxyEsterno.value else "NO",
            "proxyUrl": cfg.proxyUrl.value,
            "apiPassword": cfg.apiPassword.value,
            "timeoutProxy": cfg.timeoutProxy.value,
            "usaExtractor": "YES" if cfg.usaExtractor.value else "NO",
            "usaHlsProxy": "YES" if cfg.usaHlsProxy.value else "NO",
        }
