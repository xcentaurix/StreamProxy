...
class BaseExtractor:
    ...
    def __init__(self, request_headers=None, proxies=None, extractor_name="generic"):
    ...
    def _make_request(self, url, method="GET", headers=None, retries=2, timeout=15, **kwargs):
        if not self.session:
        ...
        for attempt in range(max(1, retries)):
            try:
                ...
                if response.status_code not in (403, 429, 500, 502, 503, 504) or attempt == retries - 1:
                    response.raise_for_status()
                    return response
                enhanced_log(
                    "[{}] HTTP {}, retry {}".format(self.extractor_name, response.status_code, attempt + 1),
                    "DEBUG",
                    "Extractor"
                )
                last_error = ExtractorError("HTTP {}".format(response.status_code))
            except Exception as exc:
                last_error = exc
                enhanced_log(
                    "[{}] richiesta fallita: {}".format(self.extractor_name, exc),
                    "DEBUG",
                    "Extractor"
                )
            time.sleep(0.25 + random.random() * 0.25)

        raise ExtractorError("Request failed for {}: {}".format(url, last_error))
