# freeshot_extractor.py - Freeshot (https://www.freeshot.live/live-tv) Extractor per Enigma2
import re
...
class FreeshotExtractor:
    ...
    def fix_video_url(self, m3u8_url, headers=None):
        try:
            if ('lovecdn.ru' in m3u8_url and 
                ...
                video_url = m3u8_url.replace('index.fmp4.m3u8', 'tracks-v1/index.fmp4.m3u8')
                enhanced_log(f"URL corretto da index.fmp4.m3u8 a tracks-v1/index.fmp4.m3u8", "INFO", "FREESHOT")
                return video_url
            return m3u8_url
    ...
    def extract(self, url, **kwargs):
        ...
        if 'lovecdn.ru' in url.lower() and url.startswith('http'):
            enhanced_log(f"FreeshotExtractor: URL lovecdn.ru diretto: {url[:100]}...", "INFO", "FREESHOT")
            ...
            channel_path = parsed.path.split('/')[1] if len(parsed.path.split('/')) > 1 else 'SkySport24IT'
        ...
        if 'popcdn.day/player/' in url.lower() and url.startswith('http'):
            enhanced_log(f"FreeshotExtractor: URL player già risolto: {url}", "INFO", "FREESHOT")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if m3u8_url:
                        ...
                        enhanced_log(f"FreeshotExtractor: M3U8 estratto -> {m3u8_url}", "INFO", "FREESHOT")
                ...
                enhanced_log(f"Errore estrazione da pagina player: {e}", "DEBUG", "FREESHOT")
            
            # Fallback
            channel_code = url.rstrip('/').split('/')[-1]
            m3u8_url = f"https://popcdn.day/stream/{channel_code}/index.fmp4.m3u8"
            m3u8_url = self.fix_video_url(m3u8_url, self.base_headers)
            enhanced_log(f"FreeshotExtractor: M3U8 fallback -> {m3u8_url}", "INFO", "FREESHOT")
        ...
        if 'freeshot.live' in url.lower():
            ...
            if len(parts) >= 3:
                ...
                if channel_code:
                    enhanced_log(f"ID: {channel_id} -> Canale: {channel_code}", "DEBUG", "FREESHOT")
                else:
                    normalized_name = channel_name.replace('-', '').lower()
                    channel_code = self.channel_map.get(normalized_name)
                    if channel_code:
                        enhanced_log(f"Nome: {channel_name} -> {channel_code}", "DEBUG", "FREESHOT")
                    else:
                        channel_code = channel_name
                        enhanced_log(f"Fallback: {channel_name} -> {channel_code}", "DEBUG", "FREESHOT")
        
        if channel_code:
            target_url = f"https://popcdn.day/go.php?stream={quote(channel_code)}"
        ...
        enhanced_log(f"FreeshotExtractor: Risoluzione {target_url} (codice: {channel_code})", "INFO", "FREESHOT")
        
        try:
            response = self.session.get(target_url, timeout=15)
            if response.status_code != 200:
                raise ExtractorError(f"Freeshot request failed: {response.status_code}")
            ...
            enhanced_log(f"FreeshotExtractor: Risolto -> {m3u8_url}", "INFO", "FREESHOT")
            enhanced_log(f"FreeshotExtractor: Iframe URL -> {iframe_url}", "DEBUG", "FREESHOT")
            ...
            enhanced_log(f"FreeshotExtractor: Stream type -> fMP4", "DEBUG", "FREESHOT")
            return result
        except Exception as e:
            enhanced_log(f"FreeshotExtractor error: {e}", "ERROR", "FREESHOT")
            import traceback
            enhanced_log(f"FreeshotExtractor traceback: {traceback.format_exc()}", "DEBUG", "FREESHOT")
