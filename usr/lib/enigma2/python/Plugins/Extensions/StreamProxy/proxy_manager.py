        ...
        try:
            ...
            if hasattr(server, 'start_proxy_server'):
                try:
                    ...
                    # Verify that the server actually started with progressive
                    # timeout
                    max_attempts = 5
                    for attempt in range(max_attempts):
                        try:
                            ...
                            if result == 0:
                                self.running = True
                                enhanced_log(
                                    "[OK] Proxy server started and verified (attempt %d)" %
                                    (attempt + 1), "INFO", "proxy_manager")
                                return True
                        except Exception as e:
                            enhanced_log(
                                "Connection verification error (attempt %d): %s" %
                                (attempt + 1, str(e)), "WARNING", "proxy_manager")
                        finally:
                            try:
                                sock.close()
                            except BaseException:
    ...
    def _check_server_status(self):
        ...
        try:
            ...
            if self._retries < self._max_retries:
                enhanced_log(
                    "Server not ready, new attempt (%d/%d)..." %
...
def start_proxy() -> bool:
    ...
    enhanced_log(
        "[INFO] Attempting to start proxy server",
        "INFO",
        "proxy_manager")
