...
class TVTapBouquetManager:
            ...
            for pattern in self.bouquet_paths:
                for bouquet_file in glob.glob(pattern):
                    try:
                        ...
                        enhanced_log(f"Scansionato bouquet: {bouquet_file} - {len(channels)} canali TVTap", "DEBUG")
                    except Exception as e:
                        enhanced_log(f"Errore scansione {bouquet_file}: {e}", "WARNING")
            
            with self.lock:
                self.tvtap_channels = found_channels
            
            enhanced_log(f"Scansione completata: {len(found_channels)} canali TVTap trovati", "INFO")
    ...
    def _parse_bouquet_file(self, bouquet_file):
        ...
        try:
            ...
            # Cerca linee SERVICE
            service_lines = re.findall(r'#SERVICE[^\n]*', content, re.IGNORECASE)
            ...
            for line in service_lines:
                try:
                    ...
                    if channel_info and self._is_tvtap_channel(channel_info):
                        ...
                        enhanced_log(f"Canale TVTap trovato: {channel_info.get('name', 'Unknown')}", "DEBUG")
                except Exception as e:
                    enhanced_log(f"Errore parsing linea service: {e}", "DEBUG")
            
        except Exception as e:
            enhanced_log(f"Errore lettura bouquet {bouquet_file}: {e}", "WARNING")
    ...
    def _is_tvtap_channel(self, channel_info):
        ...
        for pattern in self.tvtap_patterns:
            if re.search(pattern, url, re.IGNORECASE) or re.search(pattern, name, re.IGNORECASE):
                return True
    ...
    def resolve_tvtap_url(self, original_url, channel_name=None):
        ...
        with self.lock:
            ...
            # Risolvi nuovo URL
            return self._resolve_fresh_url(original_url, channel_name, cache_key)
    ...
    def _resolve_fresh_url(self, original_url, channel_name, cache_key):
        ...
        try:
            ...
            if not channel_id:
                enhanced_log("Impossibile determinare ID canale TVTap", "ERROR")
                return None
            ...
            if not new_stream_url:
                enhanced_log(f"Impossibile ottenere stream per canale TVTap {channel_id}", "ERROR")
                return None
            ...
            enhanced_log(f"URL TVTap risolto: {new_domain} - authSign: {new_authsign[:10] if new_authsign else 'None'}...", "INFO")
    ...
    def _start_background_refresh(self, original_url, cache_key):
        ...
        def refresh_worker():
            try:
                enhanced_log(f"Background refresh URL TVTap: {original_url[:50]}...", "DEBUG")
    ...
    def get_proxy_url_for_service(self, service_ref, base_proxy_url="http://127.0.0.1:7860"):
        """
        ...
        resolved_data = self.resolve_tvtap_url(original_url, channel_info.get('name'))
        ...
        enhanced_log(f"Proxy URL TVTap generato per: {channel_info.get('name', 'Unknown')}", "DEBUG")
        return proxy_url
    ...
    def cleanup_expired_cache(self):
        ...
        with self.lock:
            ...
            if expired_keys:
                enhanced_log(f"Pulite {len(expired_keys)} cache TVTap scadute", "INFO")
    ...
    def force_refresh_all(self):
        ...
        with self.lock:
            ...
            enhanced_log(f"Refresh forzato per {len(urls_to_refresh)} URL TVTap", "INFO")
            
            for original_url, cache_key in urls_to_refresh:
                try:
                    self._resolve_fresh_url(original_url, None, cache_key)
                except Exception as e:
                    enhanced_log(f"Errore refresh {original_url}: {e}", "ERROR")
...
def get_tvtap_proxy_url_for_service(service_ref, base_proxy_url="http://127.0.0.1:7860"):
    """
    ...
    """
    return tvtap_bouquet_manager.get_proxy_url_for_service(service_ref, base_proxy_url)
