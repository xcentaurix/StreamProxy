...
class TVTapWMSManager:
    ...
    def calculate_expiry_from_authsign(self, authsign):
        ...
        try:
            ...
            if 'server_time' in decoded_info and 'valid_minutes' in decoded_info:
                ...
                try:
                    ...
                    if match:
                        ...
                        enhanced_log(
                            f"✅ Server time parsed: {server_time}", "DEBUG")
                    else:
                    ...
                    enhanced_log(
                        f"❌ Parsing server_time fallito: {e}, token non valido", "ERROR")
                ...
                if age_hours > 24:
                    enhanced_log(
                        f"❌ wmsAuthSign TROPPO VECCHIO! Age: {
                            age_hours:.1f}h, forzo refresh", "WARNING")
                    return now

                # Verifica se è già scaduto
                if real_expires_at <= now:
                    enhanced_log(
                        f"⚠️ wmsAuthSign SCADUTO! Scadenza: {real_expires_at}, Ora: {now}",
                        "WARNING")
                    return now

                enhanced_log(
                    f"✅ wmsAuthSign valido fino a: {real_expires_at}", "DEBUG")
            ...
            if 'valid_minutes' in decoded_info:
                ...
                enhanced_log(
                    f"Scadenza calcolata da validminutes (fallback): {expires_at} ({valid_minutes} min)",
                    "DEBUG")
                return expires_at
    ...
    def resolve_wms_tvtap_url(self, original_url, channel_name=None):
        ...
        """
        enhanced_log(
            f"Risoluzione URL TVTap WMS: {original_url[:100]}...", "INFO")
    ...
    def _resolve_fresh_wms_url(self, original_url, channel_name, cache_key):
        ...
        enhanced_log(
            f"Risoluzione fresh URL TVTap WMS: {original_url}",
            "INFO")
        ...
        try:
            ...
            if existing_authsign:
                ...
                if time_to_expiry > 120:
                    enhanced_log(
                        f"✅ wmsAuthSign esistente valido per {
                            time_to_expiry /
                            60:.1f} min, uso quello",
                        "INFO")
            ...
            enhanced_log(
                "🔄 Forzo rigenerazione wmsAuthSign tramite API TVTap",
                "INFO")
            ...
            if not fresh_stream_url or not self.is_wms_tvtap_url(
                    fresh_stream_url):
                enhanced_log(
                    "❌ Impossibile ottenere stream da API TVTap", "ERROR")
                return None
            ...
            enhanced_log(
                f"🔑 wmsAuthSign fresco: {wms_authsign[:20]}..., scadenza: {expires_at}", "INFO")
    ...
    def _start_background_refresh(self, original_url, channel_name, cache_key):
        ...
        def refresh_worker():
            try:
                enhanced_log(
                    f"Background refresh URL TVTap WMS: {original_url[:50]}...", "DEBUG")
                ...
                with self.lock:
                    if cache_key in self.url_cache:
                        ...
                        enhanced_log(
                            "Cache TVTap WMS rimossa per refresh proattivo", "DEBUG")

                # Forza rigenerazione chiamando resolve_wms_tvtap_url
                # Questo attiverà la logica di verifica e rigenerazione
                # wmsAuthSign
                refreshed_data = self.resolve_wms_tvtap_url(
                    original_url, channel_name)
                if refreshed_data and refreshed_data.get('was_refreshed'):
                    enhanced_log(
                        "✅ Background refresh completato con successo", "INFO")
                else:
                    enhanced_log(
                        "⚠️ Background refresh non ha rigenerato wmsAuthSign",
                        "WARNING")
        ...
        try:
            ...
            for part in path_parts:
                if part.startswith('it-') and '.stream' in part:
                    ...
                    enhanced_log(
                        f"Nome canale estratto (pattern it-): {channel_name}", "DEBUG")
                    return channel_name
            ...
            if path_parts:
                ...
                if filename and len(filename) > 2:
                    channel_name = filename.replace(
                        '-', ' ').replace('_', ' ').title()
                    enhanced_log(
                        f"Nome canale estratto (filename): {channel_name}", "DEBUG")
                    return channel_name
            ...
            try:
                ...
                for ch in channels:
                    ...
                    if ch_name_lower in url_lower:
                        enhanced_log(
                            f"Nome canale trovato (match statico): {
                                ch['name']}", "DEBUG")
                        return ch['name']
            except Exception as e:
            ...
            enhanced_log(
                "Impossibile estrarre nome canale dall'URL",
                "WARNING")
            return None
    ...
    def _get_fresh_tvtap_stream(self, channel_name):
        ...
        try:
            ...
            enhanced_log(
                f"🔄 Ricerca nuovo stream per canale: {channel_name}",
                "INFO")
            ...
            if not channels:
                enhanced_log(
                    "❌ Nessun canale disponibile da TVTap API", "ERROR")
                return None

            enhanced_log(
                f"✅ Ricevuti {
                    len(channels)} canali da TVTap API",
                "DEBUG")

            # Trova canale per nome
            enhanced_log(
                f"🔍 Ricerca canale '{channel_name}' nella lista...",
                "DEBUG")
            found_channel = find_channel_by_name(channel_name, channels)
            if not found_channel:
                enhanced_log(
                    f"❌ Canale '{channel_name}' non trovato nella lista TVTap",
                    "WARNING")
                # Log primi 5 canali per debug
                sample = [ch.get('name', 'N/A') for ch in channels[:5]]
                enhanced_log(
                    f"📋 Canali disponibili (sample): {sample}", "DEBUG")
                return None

            channel_id = found_channel.get('id')
            if not channel_id:
                enhanced_log(
                    f"❌ ID non trovato per canale '{channel_name}'", "ERROR")
                return None
            ...
            # Ottieni stream URL
            enhanced_log(
                f"📡 Richiesta stream URL per ID {channel_id}...",
                "DEBUG")
            stream_url = get_tvtap_stream(channel_id)
            if stream_url:
                enhanced_log(
                    f"✅ Nuovo stream URL ottenuto: {stream_url[:80]}...", "INFO")
                return stream_url
            else:
                enhanced_log(
                    f"❌ Impossibile ottenere stream URL per {channel_name}",
                    "ERROR")
                return None
    ...
    def get_proxy_url_for_wms_url(
        ...
        enhanced_log(
            f"Proxy URL TVTap WMS generato per: {
    ...
    def cleanup_expired_cache(self):
        ...
        with self.lock:
            ...
            if expired_keys:
                enhanced_log(
                    f"Pulite {
                        len(expired_keys)} cache TVTap WMS scadute",
                    "INFO")
    ...
    def force_refresh_all(self):
        ...
        with self.lock:
            ...
            enhanced_log(
                f"Cache TVTap WMS pulita: {old_count} URL rimossi",
                "INFO")
