            ...
            try:
                ...
                with urllib.request.urlopen(test_url, timeout=2) as response:
                    if response.getcode() == 200:
                        enhanced_log(f"Server proxy attivo sulla porta {port}", "INFO", "UTILS")
                        return True
            except:
                enhanced_log(f"La porta {port} è in uso ma non risponde come server proxy", "WARNING", "UTILS")
                return False
        
        enhanced_log(f"Server proxy non in ascolto sulla porta {port}", "WARNING", "UTILS")
        return False
...
def restart_server():
    ...
    try:
        ...
        if proxy_manager.start_proxy():
            enhanced_log("✅ Server proxy riavviato con successo", "INFO", "UTILS")
            
            # Verifica che il server sia effettivamente in ascolto
            time.sleep(1)
            if check_server_status():
                enhanced_log("✅ Server proxy verificato dopo il riavvio", "INFO", "UTILS")
                return True
            else:
                enhanced_log("❌ Server proxy non risponde dopo il riavvio", "ERROR", "UTILS")
                return False
        else:
            enhanced_log("❌ Errore nel riavvio del server proxy", "ERROR", "UTILS")
            return False
    except Exception as e:
...
def wait_for_server_start(max_attempts=5, delay=1):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            with socket.create_connection(("127.0.0.1", proxy_port), timeout=2):
                ...
                try:
                    ...
                    with urllib.request.urlopen(test_url, timeout=2) as response:
                        if response.getcode() == 200:
                            enhanced_log(f"Server proxy avviato (tentativo {attempt + 1})", "INFO", "UTILS")
                            return True
                except:
                    pass
        except:
        ...
        enhanced_log(f"In attesa del server... ({attempt + 1}/{max_attempts})", "INFO", "UTILS")
        time.sleep(delay)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--check', action='store_true', help='Verifica lo stato del server')
    parser.add_argument('--restart', action='store_true', help='Riavvia il server')
